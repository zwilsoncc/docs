...
export default class UseTeamInfo extends React.Component {
  ...
  async fetchTeams() {
    ...
    if (!teamsData) {
      ...
      // Cache teams data. So, we don't need to fetch them again and again
      window.__zeit_teams_data = {
        data: teamsData,
        timestamp: Date.now()
      }
    }
    ...
    const teams = teamsList(teamsData.teams, user, team)
    this.setState({ teams })
  }

  UNSAFE_componentWillReceiveProps(nextProps) {
    let teamsData = this.teamsData
    if (!teamsData) {
      teamsData = { teams: [] }
    }

    const teams = teamsList(teamsData.teams, nextProps.user, nextProps.team)
    this.setState({ teams })
  }
