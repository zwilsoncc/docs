export default [
  {
    name: 'Introduction',
    href: '/docs/v2/introduction',
    aliases: ['/docs', '/docs/v2'],
    highlightCategory: true
  },
  {
    name: 'Git Integration',
    href: '/docs/v2/git-integration',
    highlightCategory: true
  },
  {
    name: 'Build Step',
    href: '/docs/v2/build-step',
    highlightCategory: true
  },
  {
    name: 'Custom Domains',
    href: '/docs/v2/custom-domains',
    highlightCategory: true
  },
  {
    name: 'Serverless Functions',
    posts: [
      {
        name: 'Introduction',
        href: '/docs/v2/serverless-functions/introduction'
      },
      {
        name: 'Supported Languages',
        href: '/docs/v2/serverless-functions/supported-languages'
      },
      {
        name: 'Edge Caching',
        href: '/docs/v2/serverless-functions/edge-caching'
      },
      {
        name: 'Function Environment Variables & Secrets',
        href: '/docs/v2/serverless-functions/env-and-secrets'
      }
    ]
  },
  {
    name: 'Network',
    posts: [
      {
        name: 'Overview',
        href: '/docs/v2/network/overview'
      },
      {
        name: 'Caching',
        href: '/docs/v2/network/caching'
      },
      {
        name: 'Headers',
        href: '/docs/v2/network/headers'
      },
      {
        name: 'Encryption',
        href: '/docs/v2/network/encryption'
      },
      {
        name: 'Status Codes',
        href: '/docs/v2/network/status-codes'
      },
      {
        name: 'Directory Listing',
        href: '/docs/v2/network/directory-listing'
      },
      {
        name: 'Compression',
        href: '/docs/v2/network/compression'
      }
    ]
  },
  {
    name: 'Advanced',
    posts: [
      {
        name: 'Now for GitHub',
        href: '/docs/v2/advanced/now-for-github'
      },
      {
        name: 'Now for GitLab',
        href: '/docs/v2/advanced/now-for-gitlab'
      },
      {
        name: 'Configuration',
        href: '/docs/v2/advanced/configuration'
      },
      {
        name: 'Global Configuration',
        href: '/docs/v2/advanced/global-configuration'
      },
      {
        name: 'Routes',
        href: '/docs/v2/advanced/routes'
      },
      {
        name: 'Deploy Hooks',
        href: '/docs/v2/advanced/deploy-hooks'
      },
      {
        name: 'Builders Reference',
        href: '/docs/v2/advanced/builders'
      },
      {
        name: 'Builders Developer Reference',
        href: '/docs/v2/advanced/builders-developer-reference'
      },
      {
        name: 'Deployment Concepts',
        overview: '/docs/v2/advanced/concepts/overview',
        posts: [
          {
            name: 'Overview',
            href: '/docs/v2/advanced/concepts/overview'
          },
          {
            name: 'Lifecycle',
            href: '/docs/v2/advanced/concepts/lifecycle'
          },
          {
            name: 'Immutability',
            href: '/docs/v2/advanced/concepts/immutability'
          },
          {
            name: 'URLs',
            href: '/docs/v2/advanced/concepts/urls'
          },
          {
            name: 'Special Paths',
            href: '/docs/v2/advanced/concepts/special-paths'
          },
          {
            name: 'Lambdas',
            href: '/docs/v2/advanced/concepts/lambdas'
          },
          {
            name: 'CDN and Global Distribution',
            href: '/docs/v2/advanced/concepts/cdn-and-global-distribution'
          }
        ]
      },
      {
        name: 'Domains',
        posts: [
          {
            name: 'Transferring a Domain',
            href: '/docs/v2/advanced/domains/transferring-a-domain'
          },
          {
            name: 'Managing DNS',
            href: '/docs/v2/advanced/domains/dns'
          }
        ]
      },
      {
        name: 'Platform',
        posts: [
          {
            name: 'Overview',
            href: '/docs/v2/advanced/platform/overview'
          },
          {
            name: 'Regions and Providers',
            href: '/docs/v2/advanced/platform/regions-and-providers'
          },
          {
            name: 'Projects',
            href: '/docs/v2/advanced/platform/projects'
          },
          {
            name: 'Users and Teams',
            href: '/docs/v2/advanced/platform/users-and-teams'
          },
          {
            name: 'Add-Ons',
            href: '/docs/v2/advanced/platform/add-ons'
          },
          {
            name: 'Glossary',
            href: '/docs/v2/advanced/platform/glossary'
          },
          {
            name: 'Changes in ZEIT Now 2.0',
            href: '/docs/v2/advanced/platform/changes-in-now-2-0'
          },
          {
            name: 'Frequently Asked Questions',
            href: '/docs/v2/advanced/platform/frequently-asked-questions'
          }
        ]
      }
    ]
  }
]
