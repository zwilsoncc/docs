...
- The [`distDir` option](/docs/v2/deployments/official-builders/static-build-now-static-build/#configuring-the-dist-directory) in the build step instructs Now to identify `public` as the static folder and build output directory.
...
When your app is deployed, you will receive a deployment URL like the following: [https://my-hugo-project-9c5t1giwu.now.sh/](https://my-hugo-project-9c5t1giwu.now.sh/)

## Bonus: Cache Your Hugo Assets

Caching makes your site fast and reduces bandwidth consumption for your users and cost for you. You can configure Now's routes within your `now.json` configuration file to set up caching headers.
...
```json
{
  ...
  "routes": [
    {"src": "^/dist/js/(.*)", "headers": {"cache-control": "public,max-age=31536000,immutable"} },
    {"src": "^/dist/css/(.*)", "headers": {"cache-control": "public,max-age=31536000,immutable"} },
