...
Next, click the **App Keys** tab. Copy these values so that you can save them as [<ProductShortName /> Secrets](/docs/v2/serverless-functions/env-and-secrets/#adding-secrets) and then provide them to your app as [environment variables](/docs/v2/serverless-functions/env-and-secrets/#providing-environment-variables).
...
Using [<ProductShortName /> CLI](/download), add the following [<ProductShortName /> Secrets](https://zeit.co/docs/v2/build-step#using-environment-variables-and-secrets) to your account and expose them as environment variables.

<Snippet dark text={`now secrets add channels-app-id [Your Channel's app ID]`} />
<Caption>
  Adding the <InlineCode>channels-app-id</InlineCode> as a <ProductShortName /> Secret.
</Caption>

<Snippet dark text={`now secrets add channels-app-secret [Your Channel's app Secret]`} />
<Caption>
  Adding the <InlineCode>channels-app-secret</InlineCode> as a <ProductShortName /> Secret.
</Caption>
