...
export const meta = {
  ...
  description: `Migrate from a Next.js app with a custom server to a serverless Next.js app using ${PRODUCT} routes.`,
...
<Note type="warning">
  The custom server method will not work with the <ProductName /> 2.0 platform.
</Note>

### The New Routes Method

With [<ProductName/>](https://zeit.co/now) and the serverless approach, each [Next.js page](https://nextjs.org/docs/basic-features/pages) is a separate entry point, making the sole custom server entry point a thing of the past for the good of performance and stability. Fortunately, **<ProductName/> 2.0 offers a solution**, handling the configuration of routes for you.
...
If you are yet to set up routing for [your Next.js app](https://nextjs.org/docs/#setup), this brief example describes how to use Next.js routing with [<ProductName/>](https://zeit.co/now) to support custom URLs.
