...
By making use of Next.js path segments with Now, [dynamic routing](https://nextjs.org/docs#routing) is achieved through the file structure. No longer are custom servers or additional configuration required, **simplifying the process of building your app significantly**.
...
With the Next.js app setup complete, we can deploy our app [using ZEIT Now](/docs) with **just a single command**.

<Snippet dark text="now" />

<Caption>
  Deploying our app with the <InlineCode>now</InlineCode> command.
</Caption>

If you are using either [Now for GitHub](/docs/v2/integrations/now-for-github/) or [Now for GitLab](/docs/v2/integrations/now-for-gitlab/); pushing your code to your repository is all you need to do for Now to automatically deploy your app.
