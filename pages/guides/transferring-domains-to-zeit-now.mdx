...
export const meta = {
  title: `Transferring Domains to ${PRODUCT}`,
  description: `How to transfer your domain to ${PRODUCT}.`,
...
With <ProductName />, you can [assign a custom domain](/docs/v2/custom-domains#adding-a-domain) to any of your projects. If you want to transfer your domain in from an external registrar to <ProductNameSimple />, you can do that painlessly without worrying about domain and DNS configuration.

This guide will walk you through how to transfer your domain to <ProductNameSimple /> in a few steps.
...
When transferring a domain, you will have two options to choose from. Either using the [<ProductNameSimple /> Dashboard](https://zeit.co/dashboard/domains) or [Now CLI](/download).

### Option 1: Using <ProductNameWrapper name={PRODUCT_SIMPLE} /> Dashboard

Using the [<ProductNameSimple /> Dashboard](https://zeit.co/dashboard/domains), you can perform a domain transfer by entering the domain you want to transfer in, and the respective authorization code from the domain registrar via the UI like so:
...
<Image
  ...
  height={1518/2}
/>
<Caption>Transferring in a Domain via the <ProductNameSimple /> Dashboard.</Caption>
...
After activating a domain `transfer-in`, you will be informed that the DNS settings for your domain will not be transferred along with the domain. Instead, you will need to download a zone file from your registrar and upload it to <ProductNameSimple />.

You can import the zone file to <ProductNameSimple /> DNS by running the following command:
...
<Note>
  ...
  applies the default <ProductNameSimple /> DNS settings.
</Note>
...
<Note type="warning">
  ...
  registered with <ProductNameSimple /> for at least 60 days.
</Note>
