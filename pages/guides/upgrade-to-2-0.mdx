...
export const meta = {
  title: 'Upgrade to Now 2.0',
  description:
    'Upgrade to the latest iteration of the Now Platform and unlock the true meaning of serverless deployments.',
  published: '2019-07-12T20:06:26.000Z',
  authors: ['msweeneydev'],
  url: '/guides/upgrade-to-2-0',
  editUrl: 'pages/guides/upgrade-to-2-0.mdx',
  lastEdited: '2019-07-12T09:32:06.000Z'
}

The Now Platform 2.0 simplifies and streamlines the deployment of static and serverless workloads. In this guide we will walk you through the key differences between Now 1.0 and 2.0, and the improvements this brings.
...
- Multiple [**parallel builds**](/docs/v2/deployments/builds#parallelization) can be specified to compile source into [static files or serverless Lambdas](/docs/v2/deployments/builds#sources-and-outputs)
...
All static assets and caches are served from our [Global CDN](/docs/v2/deployments/concepts/cdn-and-global-distribution) **by default**. When it comes to code invocation with [Lambdas](/docs/v2/deployments/concepts/lambdas), it makes sense to think about whether the code is executed in a single region or multiple.
...
<Note hint>
  ...
  <P.B>closest to your date source</P.B>.
</Note>
...
We have decided to make this logic happen on the client side, instead of the server side. [Now CLI](/docs/v2/getting-started/installation/) will define an item in `routes` that also maps `/` as a special case.
