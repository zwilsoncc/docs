...
Create a [Secret](/docs/cli#commands/secrets) to securely store the SendGrid API key, this will be used when deploying the app with <ProductName />.
...
Lastly, to make the [Secret](/docs/cli#commands/secrets) available to the Serverless Function when deployed, create a `vercel.json` file:
