...
<Note type="warning">
  ...
  you from receiving the full benefits of ZEIT's platform and services, such as
  the automatic renewal of wildcard certificates. Furthermore, many DNS
  providers{' '}
  <b>do not have a global connection capable of low-latency geo-routing</b>,
  which is likely to <b>slow down your apps</b>.
</Note>
...
<Caption>Adding a domain to a ZEIT account using <Link href="/now">Now CLI</Link>.</Caption>
...
<Image
  src={`https://zeit-inc-res.cloudinary.com/image/upload/v1546863670/front/docs/domains/cli-add-domain.png`}
  width={1670/2}
  height={1046/2}
/>
<Caption>Adding a domain with <Link href="/docs/v2/getting-started/installation">Now CLI</Link>.</Caption>

To avoid pointing the nameservers to ZEIT Now prematurely, use the TXT method for verification.
...
[ZEIT Now](/) periodically checks for whether or not the TXT record (or nameservers in another case) has been added and then mark the domain as verified when it detects them. You can force ZEIT Now to check for the verification by using the following command:

<Snippet dark text="now domains verify <domain>" />

<Image
  src={`https://zeit-inc-res.cloudinary.com/image/upload/v1546863671/front/docs/domains/cli-verify-domain.png`}
  width={1670/2}
  height={1046/2}
/>
<Caption>Verifying a domain with <Link href="/now">Now CLI</Link>.</Caption>

## Step 2: Preparing the Domain with DNS, Certs, and Deploying to Production

Now that your domain is verified, you have more control over it with ZEIT Now. You can now generate an SSL Certificate, add any DNS Records, and deploy your app using your domain, all ready for when you switch over fully.
...
<Caption>Generating an SSL certificate for a domain and all subdomains with <Link href="/now">Now CLI</Link>.</Caption>

You will then be asked to solve a DNS challenge, required for the Certificate Authority (CA) to prove that ZEIT Now has sufficient permissions, consisting of two TXT records.
...
<Caption>
  Adding MX records to a domain with <Link href="/now">Now CLI</Link>.
</Caption>
...
<Note>
  ...
  <Link href="/docs/v2/integrations/now-for-gitlab">ZEIT Now for GitHub</Link>,
  <Link href="/docs/v2/integrations/now-for-gitlab">ZEIT Now for GitLab</Link>,
  or
  <Link href="/docs/v2/integrations/now-for-bitbucket">
