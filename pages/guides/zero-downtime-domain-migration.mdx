...
<Snippet dark text="vc certs issue \*.<domain> <domain> --challenge-only" />
...
Once these TXT records are added to your domain via your domain provider, run the `vc certs issue \*.<domain> <domain>` command again to attempt the certificate issuance.
...
The above demonstrates using the `vc dns add` command to add multiple MX record to your domain.
...
Using `vc domains inspect <domain>`, you can find your intended set of nameservers to use with your domain, in case you have forgotten them when you first added the domain.
