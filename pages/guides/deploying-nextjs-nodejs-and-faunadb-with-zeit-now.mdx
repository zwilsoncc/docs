...
Create a [<ProductShortName /> Secret](/docs/v2/serverless-functions/env-and-secrets/) to store the access key received, this will be used later when accessing the database.

<Snippet dark text="now secrets add FAUNADB_SECRET_KEY [your-access-key]" />
<Caption>
  Adding the access key as a <strong><ProductShortName /> Secret</strong>.
</Caption>
...
Create a `now.json` at the root of the project directory, this is used to make the [<ProductShortName /> Secret](https://zeit.co/docs/v2/serverless-functions/env-and-secrets/) defined in [step 2](#step-2:-create-an-access-key) available to the Serverless Function.
