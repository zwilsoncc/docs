...
- Defines a `builds` property as an array with one build step using `@now/next` to instruct Now to statically build the project and deploy the `build` directory. Using the `package.json` file as an entrypoint, [more on that below](#build-instruction).
