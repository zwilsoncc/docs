...
Using [<ProductNameShort /> CLI](/download), create [<ProductNameShort /> Secrets](/docs/v2/serverless-functions/env-and-secrets/) to securely store the API key, the project `id` from the TakeShape dashboard, and a preview secret.

<Snippet dark text="now secrets add next_example_cms_takeshape_api_key [your-takeshape-api-key]" />
<Caption>
  Adding the <InlineCode>next_example_cms_takeshape_api_key</InlineCode> as a <strong><ProductNameShort /> Secret</strong>.
</Caption>
...
<Caption>
  Adding the <InlineCode>next_example_cms_takeshape_project_id</InlineCode> as a <strong><ProductNameShort /> Secret</strong>.
</Caption>

Create a random string that can be used as a preview secret and add it using <ProductNameShort /> CLI. You can use a string similar to `MY_SECRET`, but remember to change it to make it unique.

<Snippet dark text="now secrets add next_example_cms_takeshape_preview_secret [your-random-string]" />
<Caption>
  Adding the <InlineCode>next_example_cms_takeshape_preview_secret</InlineCode> as a <strong><ProductNameShort /> Secret</strong>.
</Caption>
