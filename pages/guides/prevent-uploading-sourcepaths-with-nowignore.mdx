...
export const meta = {
  title: `Prevent Uploading Source Paths with .nowignore on ${PRODUCT}`,
  description: `Use a .nowignore file to prevent the uploading of source paths when deploying with ${PRODUCT}.`,
  ...
  lastEdited: '2020-04-03T19:15:03.000Z'
}
...
Aside from the [default exclusions](/docs/v2/build-step/#ignored-files-and-folders), all files within your project are uploaded to <ProductName/> if no source path is specified to be excluded in a [`.nowignore` configuration file](https://zeit.co/docs/v2/deployments/ignoring-source-paths).
