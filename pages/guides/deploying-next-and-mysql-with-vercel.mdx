...
export const meta = {
  ...
  name: 'Next.js + MySQL',
  type: 'app',
  lastEdited: '2020-06-10T18:46:29.000Z'
}
...
Now, add your database credentials from [step 1](#step-1:-populating-your-mysql-database) to the project as [Secrets](/docs/cli#commands/secrets) using the [<ProductShortName /> CLI](/download) to keep them secure:

<Snippet dark text="vercel secrets add MYSQL_HOST $database-hostname && vercel secrets add MYSQL_USER $database-username && vercel secrets add MYSQL_DATABASE $database-name && vercel secrets add MYSQL_PASSWORD $database-password" />
<Caption>
...
- Creates a connection to your MySQL database using credentials defined as [Secrets](/docs/cli#commands/secrets)
...
Create a [`vercel.json` file](/docs/configuration) with the following configuration to make the environment variables available to your Serverless Functions:
...
<Caption>
  An example <InlineCode>vercel.json</InlineCode> file for your project.
</Caption>
