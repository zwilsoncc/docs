...
export const meta = {
  ...
  description:
    'Migrate your websites and APIs to the ${PRODUCT} Platform with zero configuration.',
...
ZEIT Now tries to automatically detect the frontend framework you’re using for your project and configure the project settings for you. If you’d like to override the settings or specify a different framework, take a look at [our documentation on build settings](/docs/v2/build-step).

## Custom Domains

To migrate your domains to ZEIT Now, refer to the following documentation:

- [Custom Domains](/docs/v2/custom-domains)
- [Zero-Downtime Domain Migration with ZEIT Now](/guides/zero-downtime-domain-migration)
...
The only requirement for Serverless Functions on ZEIT Now, without advanced configuration, is that you place your files inside of an `/api` directory. By doing this, ZEIT Now will be able to handle both the routing and building of these files – dramatically simplifying the process of developing an API.
...
Using route parameters with your Serverless Functions is made easy with ZEIT Now. Let's first take a look at another serverful Express.js example that uses a dynamic route.
...
To convert this route for use inside of the `/api` directory with ZEIT Now, we will once again place it inside the `/teams` directory. However, this time we will name it `[member].js` - this is called a _Path Segment_.
...
Taking this into account, we can convert the above Express route for ZEIT Now with the following code:
