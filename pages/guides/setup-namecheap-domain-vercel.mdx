...
Select `TXT Record` as the record type, add `_now` as the host, and the value you were given for the record when adding your domain. You can fetch the value again using the `vc domains inspect <domain>` via the [<ProductShortName /> CLI](/docs/v2/getting-started/installation#now-cli).
...
​<ProductName /> provides an easy to use command that allows you to add DNS records from the CLI. In this example, you can configure your domain with a set of Gmail MX records via the `vc dns add` command:
...
Learn more about using the [`vc dns`](/docs/now-cli#commands/dns) command.
...
The final step of migrating your domain to <ProductName /> is to point your domain to <OrgName /> DNS with the intended nameservers set. Using `vc domains inspect <domain>`, you can always find the intended nameservers to use with your domain.
