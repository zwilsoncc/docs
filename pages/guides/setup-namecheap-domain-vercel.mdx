...
First, you need to add your Namecheap registered domain to your <OrgName /> account. To get started, use the [<ProductShortName /> CLI](/download):

<Snippet dark text="vercel domains add <domain>" />

The command above adds the domain to your <OrgName /> account. Once it has been added, the [<ProductShortName /> CLI](/download) outputs some information needed to verify ownership of the domain.
...
Select `TXT Record` as the record type, add `_now` as the host, and the value you were given for the record when adding your domain. You can fetch the value again using the `vercel domains inspect <domain>` via the [<ProductShortName /> CLI](/download).
...
Learn more about using the [`vercel dns`](/docs/cli#commands/dns) command.
