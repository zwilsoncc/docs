...
To use your MongoDB connection string without hard-coding it in your project, you can add your connection string to the project as a [<ProductShortName /> Secret](/docs/v2/deployments/environment-variables-and-secrets#securing-environment-variables-using-secrets) using the [<ProductShortName /> CLI](/download#now-cli) to keep them secure:
...
This file allows the <ProductName /> development process to use the environment variable with a value, since you are using a <ProductShortName /> Secret in production which isn't accessible outside of the secure deployment environment.
