...
Finally, set up [the `now.json` configuration file](/docs/configuration) that instructs Now to make available the [Secret](/docs/v2/deployments/environment-variables-and-secrets#securing-environment-variables-using-secrets) you defined:
...
<Note>
  ...
  in the <Link href="/docs/v2/platform/regions-and-providers">
...
Instead of deploying manually through your terminal, you can also get **automatic deployments with each git push** when you use [Now for GitHub](/github), [Now for GitLab](/gitlab), or [Now for Bitbucket](/bitbucket).
