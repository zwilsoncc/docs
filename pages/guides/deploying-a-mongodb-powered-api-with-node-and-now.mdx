...
export const meta = {
  title: `Create and Deploy a MongoDB-Powered Node.js API with ${PRODUCT}`,
  description: `Create a serverless Node.js API with data powered by MongoDB then deploy it as a serverless app with ${PRODUCT}.`,
  ...
  lastEdited: '2020-04-11T22:12:24.000Z'
}
...
To use your MongoDB connection string without hard-coding it in your project, you can add your connection string to the project as a [<ProductNameShort /> Secret](/docs/v2/deployments/environment-variables-and-secrets#securing-environment-variables-using-secrets) using the [<ProductNameShort /> CLI](/download#now-cli) to keep them secure:
...
Using <ProductNameShort /> CLI, you can start a development process from your terminal that mimics your production environment using the configuration you provided in your `now.json` file.
...
This file allows the <ProductName /> development process to use the environment variable with a value, since you are using a <ProductNameShort /> Secret in production which isn't accessible outside of the secure deployment environment.

Next, run the <ProductNameShort /> CLI development command from the terminal:

<Snippet dark text="now dev" />
<Caption>Starting <ProductNameShort /> CLI's dev process from the terminal.</Caption>

After starting the development process, <ProductNameShort /> CLI will provide you with a localhost address for where your app is running. This is commonly <http://localhost:3000> unless the port is taken.
