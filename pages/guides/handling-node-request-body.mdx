...
In this guide, we will show you how to parse a [Node.js request body](https://nodejs.org/en/docs/guides/anatomy-of-an-http-transaction/), for use inside a [Serverless Function](/docs/v2/serverless-functions/introduction/) deployed to [ZEIT Now](/now), without requiring a framework such as Express.
...
<Caption>
  ...
  with a{' '}
  <Link href="/docs/v2/deployments/official-runtimes/node-js-now-node/#helpers">
    Helper property
  </Link>
  .
</Caption>
...
The key part here is line 2. [ZEIT Now](/now) adds a number of Express like [helper properties](/docs/v2/deployments/official-runtimes/node-js-now-node/#helpers) to make wording with Node.js even easier.
...
Deploying your function to [ZEIT Now](/now) can be done with **a single command**:
...
Congratulations, now you know how to parse request bodies with [ZEIT Now](/now) in Node.js!
...
By using [helper properties](/docs/v2/deployments/official-runtimes/node-js-now-node/#helpers), the `request` is already parsed for you, allowing you to access the data immediately.
