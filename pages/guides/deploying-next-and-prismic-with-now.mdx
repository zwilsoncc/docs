...
Your blog posts are rendered, but there's no way to test them out since there's no easy way to navigate to each one currently. For that, you'll have to setup [custom routing](/docs/v2/deployments/routes/) by creating a `now.json` file.
