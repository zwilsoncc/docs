...
export const meta = {
  ...
  lastEdited: '2019-08-20T15:14:47.000Z'
}
...
You can run your application with the `npm run dev` command, and check at `localhost:3000` that the app is running correctly.
...
So far, you have been using `npm run dev` to develop locally, but what good is a blog if you don't show it off to the world. It's time to deploy it to the Now platform and enjoy your serverless web application.

You can run `npm run deploy` to launch the [Now CLI](/download) which will quickly setup and deploy your project online. Once it's done, you can check the generated url.
...
Deploy this new version to Now with `npm run deploy`. You minimal blog is now being served even faster thanks to the [Now Smart CDN](/smart-cdn). You can try adding or modifying content in your Prismic repository and it **will be served instantaneously** on your now public site.
