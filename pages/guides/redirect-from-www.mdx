...
- `alias`: The [alias property](/docs/v2/deployments/configuration#alias) defines a single or set of aliases to assign the deployment to, using [the production aliasing environment](/docs/v2/domains-and-aliases/aliasing-a-deployment/#production).
...
To push your deployment into production, deploy and alias it with the [production aliasing target](/docs/v2/domains-and-aliases/aliasing-a-deployment/#production):

<TerminalInput>now --target production</TerminalInput>

Now, when visiting either the production alias (www.your-domain.com), a [staging alias](/docs/v2/domains-and-aliases/aliasing-a-deployment/#staging), or a [deployment URL](/docs/v2/deployments/concepts/urls/), the visitor will get redirected to the location set as defined by the [`routes` property](/docs/v2/deployments/routes/) in the `now.json` file.
