...
Now provides configuration to help make redirecting easier through [the `routes` property](/docs/configuration#project/routes) within a [`now.json` file](/docs/configuration).
...
- `version`: The [version property](/docs/configuration#project/version) specifies the Now platform version to use, in this case, we are using the [latest version 2.0](/docs/v2/platform/overview/#versioning).
- `alias`: The [alias property](/docs/configuration#project/alias) defines a single or set of domains to assign the deployment to, using [the production domain](/docs/v2/custom-domains#deploying-with-your-domain).
- `routes`: The [routes property](/docs/configuration#project/routes) allows Now to route your deployment as you intend it and configure it to be routed using a source and destination, or by way of a source, status, and headers.
...
Now, when visiting either the production domain (www.your-domain.com), a [staging URL](/docs/v2/custom-domains#deploying-with-your-domain), or a deployment URL, the visitor will get redirected to the location set as defined by the [`routes` property](/docs/configuration#routes) in the `now.json` file.
