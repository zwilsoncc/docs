...
First, you need to add your Namecheap registered domain to your <ProductNameSimple /> account. To get started, use the [Now CLI](/docs/v2/getting-started/installation#now-cli):

<Snippet dark text="now domains add <domain>" />

The command above adds the domain to your <ProductNameSimple /> account. Once it has been added, the [Now CLI](/docs/v2/getting-started/installation#now-cli) outputs some information needed to verify ownership of the domain.
...
Once Namecheap is done propagating the newly changed DNS for your domain, then your domain is ready to be verified. <ProductNameSimple /> automatically checks the domain for verification. However, if you are keen on manually verifying it, then run the following command:
...
Your domain will be verified after adding the TXT record. Once <ProductNameSimple /> verifies your domain, you'll automatically receive an email. You can also manually use the command below to force a domain verification:
...
Adding your production domain to a project can be done from the [<ProductNameSimple /> Dashboard](/dashboard) by selecting the project's **Domains** tab and entering it there, you can find more information on this in the [custom domains documentation](/docs/v2/custom-domains/#adding-a-domain).
...
The final step of migrating your domain to <ProductName /> is to point your domain to <ProductNameSimple /> DNS with the intended nameservers set. Using `now domains inspect <domain>`, you can always find the intended nameservers to use with your domain.
