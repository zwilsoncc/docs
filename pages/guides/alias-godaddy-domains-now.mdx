...
ZEIT Now is dedicated to making deployment easy. If you have not yet deployed an application, take a lot at the [Deployment Basics](/docs/v2/deployments/basics) documentation or take a look at some [examples](/examples) to get started. The aliasing process will also generate a free SSL certificate for your domain.

When ready, deploy your application with the following command:

<TerminalInput>now</TerminalInput>

Following deployment, you can then alias your deployment to your domain with the following command:

<TerminalInput>{`now alias <deployment URL> <domain>`}</TerminalInput>

To make this effortless in the future, we highly recommend using [the Now for GitHub app](/docs/v2/integrations/now-for-github/) coupled with the [`alias` configuration option](/docs/v2/deployments/configuration/#alias) which will allow your application to be deployed on push to your GitHub repository and aliased automatically when merged to the main branch.
