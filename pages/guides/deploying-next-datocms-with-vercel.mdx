...
This guide walks you through creating a simple blog using Next.js's [Static Generation](https://nextjs.org/docs/basic-features/pages) feature and [DatoCMS](https://www.datocms.com/) through [DatoCMS GraphQL API](https://www.datocms.com/) and deploying the project with [Vercel](/).
...
For the frontend of this project, you will be using Next.js – the production-ready React framework. For the backend you will be using [DatoCMS](https://www.datocms.com/) – a powerful headless CMS that allows you to **rapidly create, manage and distribute content** to any platform you like.

Two of its biggest selling points are:

- **GraphQL API:** which means powerful developer tools and complete control over the data your website downloads;
- **Out-of-the-box support to responsive, progressive, lazy-loaded images** thanks to the fact that its GraphQL API already exposes [pre-computed low-quality image placeholders](https://www.datocms.com/blog/offer-responsive-progressive-lqip-images-in-2020) (LQIP, or simply blur-up placeholders), allowing webpages to get everything they need in a single request, avoiding content reflows.
...
Repeat the process to create a new model: this time we'll call it **Post**.
...
The last model we need is called **Blog**. Make sure to check the "Single instance?" option for this, as we only want to create a **single record of this type** that will hold the SEO information for the blog page.
...
From the **Content** menu at the top, select **Author** and then create a new record. Just use dummy data for the text, and you can download one image from [Unsplash](https://unsplash.com/) for the picture.
...
The last step is to fill in some SEO meta tags for our blog. Go to the **Blog** section, insert a title and a description and click "Save".
...
Next, create a set of API tokens to be used in your app, these will allow the [DatoCMS Client](https://www.datocms.com/developers/docs/javascript/tutorials/using-js-cda-sdk/) to request your posts from our API.
...
That's all the setup required for [DatoCMS](https://www.datocms.com/)! Within **just a few minutes** you have managed to create a **Content Model**, add content and generate an API token.

In the next step, you will create your [Next.js](https://nextjs.org/) app.

## Step 3: Create your Next.js App

Create a new Next.js application using [create-next-app](https://create-react-app.dev/), then run the following command and follow the wizard:
...
<Snippet
  dark
  text="cd my-app && yarn add graphql-request react-datocms && yarn dev"
/>
...
We're using an environment variable starting with `NEXT_` so that it will be automatically read by Next.js. Now we can set the env variable inside a file called `.env`:
...
Next, go to `pages/index.js` — that is, the component that renders the homepage of the project — and copy the following code:
...
First, it requires the GraphQL client we just created. Then, inside the [`getStaticProps`](https://nextjs.org/docs/basic-features/data-fetching#getstaticprops-static-generation) function, we perform the GraphQL request, so that Next.js will **pre-render this page at build time** using the props returned by it.
...
<Note>
  This is just a basic example. To learn more about Next.js Preview Mode and its
  security implications, take a look at the{' '}
...
To make the DatoCMS API token available with Vercel, first add a `now.json` file at the root of your project directory with the following code:
...
Then create a [Now Secret](/docs/v2/deployments/environment-variables-and-secrets#securing-environment-variables-using-secrets) with the command below:

<Snippet dark text="now secrets add datocms_api_token YOUR-API-TOKEN" />
<Caption>
  Adding the <InlineCode>datocms_api_token</InlineCode> secret to your Vercel account using <Link href="/docs/v2/deployments/environment-variables-and-secrets#securing-environment-variables-using-secrets">Now secrets</Link>.
</Caption>

## Step 5: Deploying the App

The Github repo with the complete example is available here: <https://github.com/zeit/next.js/tree/master/examples/cms-datocms>

You can see a preview of the result here: <https://next-blog-datocms.now.sh/>
