...
export const meta = {
  ...
  description: `A list of common questions and answers related to the ${PRODUCT_SIMPLE} Network.`,
  editUrl: 'pages/docs/v2/network/frequently-asked-questions.mdx',
  lastEdited: '2020-04-11T07:17:23.000Z'
}
...
- Your Serverless Functions have total control on how the [CDN](/docs/v2/network/caching) caches your responses using the `Cache-Control` header.
...
- Use `Origin Control: Off` along with other custom caching rules in Cloudflare, in a `now.json` file, set `s-maxage` to 0 on your routes to disable any caching from <ProductNameSimple /> CDN.
