...
export const meta = {
  ...
  description:
    'A listing and explanation of the regions and providers Now implements.',
...
An Origin Region is a Region where content originates, whereas an Edge Region can _only_ serve cached or static content. An application should generally only deploy to the Origin Regions closest to the Regions where the data is stored. If an application doesn't need to fetch data, it would perform extremely well deployed globally using `--regions all`.
...
| Region ID | Reference Location        | Provider Region (Routing)   | Provider Region (Now Lambdas) | Origin/Edge |
| --------- | ------------------------- | --------------------------- | ----------------------------- | ----------- |
...
| chs1      | Charleston, East US       | GCP us-east1                | n/a                           | Origin      |
| cle1      | Cleveland, East US        | AWS us-east-2               | AWS us-east-2                 | Edge        |
| dub1      | Dublin, Ireland           | AWS eu-west-1               | AWS eu-west-1                 | Origin      |
| gru1      | São Paulo, Brazil         | AWS sa-east-1               | AWS sa-east-1                 | Origin      |
