...
export const meta = {
  ...
  lastEdited: '2019-08-14T16:52:53.000Z'
}
...
The following headers are sent to each Now deployment and can be used to process the request before sending back a response. These headers can be read from the [Request](https://nodejs.org/api/http.html#http_message_headers) object in your [Lambda](/docs/v2/advanced/concepts/lambdas/).
...
<Note>
  The trace is where a request was processed, it is not necessarily where a
  lambda was executed, this could be different if you defined `regions` in
  your`now.json` file.
</Note>
...
Custom headers can be added dynamically to the [Response](https://nodejs.org/api/http.html#http_response_setheader_name_value) object in your [Lambda](/docs/v2/advanced/concepts/lambdas/) or they can be added statically to the [routes](/docs/v2/advanced/routes/#headers) property in your `now.json` file.
