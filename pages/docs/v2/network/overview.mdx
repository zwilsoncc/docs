...
export const meta = {
  ...
  description: `The ${PRODUCT_SIMPLE} Network layer sits in-between the internet and your deployments. It routes the users requests to the correct files of the requested deployment.`,
  editUrl: 'pages/docs/v2/network/overview.mdx',
  lastEdited: '2020-04-11T07:12:03.000Z'
}

The <ProductNameSimple /> Network layer sits in-between the internet and your <ProductName /> deployments. This type of network topology is sometimes called a [Content Delivery Network](https://en.wikipedia.org/wiki/Content_delivery_network) or CDN.

The <ProductNameSimple /> Network has many responsibilities, but most importantly, it is responsible for routing requests to the correct **Serverless Function** or **Static File** output of your **Builds**. Since the <ProductNameSimple /> network is a CDN with the ability to execute functions at the edge, the <ProductNameSimple /> Network is a [Smart CDN](/smart-cdn).
...
The <ProductNameSimple /> Network has multiple layers of caches to ensure that your <ProductName /> deployments are fast around the world. At a high level, the layers of the cache are as follows:
