...
<Example>
<P.B>Usage Example:</P.B>
<PDIV>The following example will pass the `MY_KEY` static env to all <GenericLink href="/docs/v2/deployments/builds">Builds</GenericLink> and `SECRET` resolved from the `my-secret-name` <GenericLink href="/docs/v2/deployments/environment-variables-and-secrets">secret</GenericLink> dynamically.</PDIV>
<Code>{`{
  "build": {
    "env": {
      "MY_KEY": "this is the value",
