...
<Note>It is important to remember that the <InlineCode>src</InlineCode> property is a glob. In the above example, this will only use <InlineCode>@now/static</InlineCode> for the HTML files in the root directory. If you need it to be used throughout subdirectories, you need the appropriate glob syntax: <InlineCode>**/*.html</InlineCode>.</Note>
