...
<Caption>
  ...
  <InlineCode>routes</InlineCode> properties that rewrite paths to another
  upon request.
</Caption>

<Note>
  You can point the <InlineCode>dest</InlineCode> path to any URL, Now hosted lambda, or even
  non-Now hosted URLs as shown in the code above. If you don't perform any rewriting, you can safely remove <InlineCode>dest</InlineCode>.
</Note>
...
<Caption>
  This will route to <InlineCode>/about</InlineCode> without rewriting, but routes like this are usually redundant with <GenericLink href="#wildcard-routes">handle filesystem</GenericLink>.
</Caption>
...
<Caption>
  In this case, the <InlineCode>Cache-Control</InlineCode>{' '} header will be applied to any route starting with <InlineCode>/blog</InlineCode>.
</Caption>
...
<Caption>
  A <InlineCode>now.json</InlineCode> file where filesystem routes are explicitly defined.
</Caption>
...
<Caption>
  A <InlineCode>now.json</InlineCode> file, using handle filesystem to route to filesystem routes.
</Caption>
