...
When you make a deployment, by running `now` with our [CLI](/docs/v2/gettings-started/installation#now-cli), making an [API](/api) call or with our [GitHub integration](/docs/v2/integrations/now-for-github), we synchronize the source code and optionally [build them](/docs/v2/deployments/builds) to your specification.
...
The filesystem is writable inside `/tmp` and persisted if the underlying [container is re-used](/docs/v2/platform/regions-and-providers). The `/tmp` directory is writable for up to `512mb` or storage.
