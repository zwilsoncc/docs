...
export const meta = {
  title: 'Lambdas',
  description: 'Serverless functions that are invoked upon request and serve a dynamic response, easily deployed with ZEIT Now.'
}

When you make a deployment, by running `now` with our [CLI](cli), making an [API](api) call or with our [GitHub integration](link), we synchronize the source code and optionally [build them](/docs/v2/deployments/builds) to your specification.
...
- Ensures smaller package sizes which in term make **cold bootup times very fast**
...
### CPU / Memory
Now configures lambdas to have **3008MB** of available memory for builds, and the output lambdas to have **1024MB** of available memory. The amount of CPU allocated is proportional to the max memory limit.
...
Although this limit is in place, each [Builder](/docs/v2/deployments/builders/overview) may have a customised limit for the maximum lambda bundle size due to the nature of the underlying technology, which can also be configured by the user, whether the Builder has a custom limit or not.
...
<Caption>Using the <GenericLink href="/docs/v2/deployments/official-builders/node-js-now-node">@now/node Builder</GenericLink> and configuring the <InlineCode>maxLambdaSize</InlineCode> within a <GenericLink href="/docs/v2/deployments/configuration#builds">now.json configuration file</GenericLink>.</Caption>
</Example>

### Maximum Execution Duration
A lambda can execute for at most **15 minutes**. This is not configurable as of now. If the limit is reached for a particular invocation, it will result in an error [504](/docs/v2/routing/status-codes).
...
### Filesystem
The filesystem is writable inside `/tmp` and persisted if the underlying [container is re-used](/docs/v2/platform/regions-and-providers). The `/tmp` directory is writeable for up to `512mb` or storage.
