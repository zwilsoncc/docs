...
The configuration above will pass the static env keys; `MY_KEY`, and `SECRET` to all [builds](/docs/v2/deployments/builds/). The `SECRET` env key is dynamically settled from the `my-secret-name` [secret](#securing-environment-variables-using-secrets).
