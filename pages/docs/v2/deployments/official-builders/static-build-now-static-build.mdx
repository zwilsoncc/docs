...
<Note>
  At the moment, <InlineCode>@now/static-build</InlineCode> only supports build
  processes that can be expressed using Node.js and npm dependencies.
</Note>
...
<TerminalInput>yarn add hugo-cli</TerminalInput>
...
The `src` of each build you define with `@now/static-build` must be a `package.json` file.
