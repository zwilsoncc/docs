...
export const meta = {
  ...
  lastEdited: '2019-03-24T23:18:11.000Z'
}
...
When using a `Cargo.toml` file as an entry point for this Builder, Now will use `cargo read-manifest` to build each binary within the project. It means that **[cargo workspaces](https://doc.rust-lang.org/book/ch14-03-cargo-workspaces.html) are not supported as an entry point for now.** See [cargo workspace workaround](#cargo-workspaces-workaround)
