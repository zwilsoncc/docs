...
If you need to preserve the current nameservers or a domain, or need to use an alternative method of pointing to Now, you can do so after [TXT verification](#option-2:-using-a-txt-dns-record) by creating a CNAME record with your domain provider with the value of `alias.zeit.co`.
