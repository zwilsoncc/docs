...
export const meta = {
  ...
  lastEdited: '2020-05-28T23:06:10.000Z'
}
...
When running the `vercel dev` command, you may experience an `npm install` error. This happens when the dev command invokes `npm` to install Runtimes that are defined in your `vercel.json` file.
