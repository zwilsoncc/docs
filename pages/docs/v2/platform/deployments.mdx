...
To make a [preview deployment](/docs/v2/platform/deployments/#preview), use the `vc` command:

<Snippet dark text="vc" />
<Caption>Making a preview deployment with the <InlineCode>vc</InlineCode> command.</Caption>
...
To make a [production deployment](/docs/v2/platform/deployments/#preview), use the `vc --prod` command:

<Snippet dark text="vc --prod" />
<Caption>Making a production deployment with the <InlineCode>vc</InlineCode> command.</Caption>
...
**Preview deployments** are the default for all deployments. Each time you push to a branch or make a deployment using the `vc` command, this is a **preview deployment**.
...
**Production deployments** are made in two different circumstances. Each time you merge to the default branch (commonly `master`) or make a deployment using the `vc --prod` command, this is a **production deployment**.
...
- **Force a new deployment** by instructing your client to do so.
  - For [<ProductShortName /> CLI](/download) you can run `vc --force`
