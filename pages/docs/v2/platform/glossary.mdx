...
export const meta = {
  ...
  lastEdited: '2020-01-13T16:53:48.000Z'
}

These are the terms we encourage you to learn in order to deeply understand **ZEIT Now**. However, every occurrence of one of these terms is also linked back to this very page, so that you can always jump right to the explanation if needed.

## General Terms

### Deployment

A collection of files that were uploaded to **ZEIT Now**.

#### Preview Deployments

Preview Deployments are the default type of deployment. They are made available by pushing to a git branch through a Git Integration, or by using the `now` command from Now CLI.

Learn more about [Preview Deployments from the Deployments Concept documentation](/docs/v2/platform/deployments#preview).

#### Production Deployments

Production Deployments are a promoted type of Deployment, made available by merging into a default branch through a Git Integration, or by using the `now --prod` command from Now CLI
...
The company behind the product **ZEIT Now**.
...
The command line interface client for **ZEIT Now**.
