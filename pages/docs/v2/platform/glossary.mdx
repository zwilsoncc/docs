...
export const meta = {
  ...
  lastEdited: '2020-05-13T14:57:56.000Z'
}
...
Preview Deployments are the default type of deployment. They are made available by pushing to a git branch through a Git Integration, or by using the `vercel` command from <ProductShortName /> CLI.

Learn more about [Preview Deployments from the Deployments Concept documentation](/docs/v2/platform/deployments#preview).

#### Production Deployments

Production Deployments are a promoted type of Deployment, made available by merging into a default branch through a Git Integration, or by using the `vercel --prod` command from <ProductShortName /> CLI
