...
Preview Deployments are the default type of deployment. They are made available by pushing to a git branch through a Git Integration, or by using the `vc` command from <ProductShortName /> CLI.

Learn more about [Preview Deployments from the Deployments Concept documentation](/docs/v2/platform/deployments#preview).

#### Production Deployments

Production Deployments are a promoted type of Deployment, made available by merging into a default branch through a Git Integration, or by using the `vc --prod` command from <ProductShortName /> CLI
