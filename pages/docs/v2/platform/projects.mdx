import Doc from '~/components/layout/docs'
import { InlineCode } from '~/components/text/code'
import Snippet from '~/components/snippet'
import Link from '~/components/text/link'
import Caption from '~/components/text/caption'
import Note from '~/components/text/note'
import { Image } from '~/components/media'

export const meta = {
  title: 'Projects',
  description: 'Setting up projects with a list of deployments on ZEIT Now.',
  editUrl: 'pages/docs/v2/platform/projects.mdx',
  lastEdited: '2020-03-20T03:35:23.000Z'
}

To deploy on ZEIT Now, you need to create a **Project**. A Project is a logical structure that groups [deployments](/docs/v2/platform/deployments) and [custom domains](/docs/v2/custom-domains). Each project can be associated to a [GitHub](/docs/v2/git-integrations/zeit-now-for-github), [GitLab](/docs/v2/git-integrations/zeit-now-for-gitlab), or [Bitbucket](/docs/v2/git-integrations/zeit-now-for-bitbucket) repository.

## Creating a Project

You can create a new project from the ZEIT Dashboard by clicking on [the New Project button](/new).

<Image
  src={`${process.env.ASSETS}/docs/platform/projects/new-project.png`}
  width={1744 / 2}
  height={516 / 2}
  shadow
/>
<Caption>Creating a new project from the ZEIT Dashboard.</Caption>

You can either [import from an existing Git repository](/docs/v2/git-integrations) or use one of our templates.

## Project Settings

To edit project settings, choose a project from the [dashboard](https://zeit.co/dashboard):

<Image
  src={`${process.env.ASSETS}/docs/common/select-project.png`}
  width={1102 / 2}
  height={410 / 2}
  shadow
/>
<Caption>Selecting the project from the ZEIT Dashboard.</Caption>

Then, select the **Settings** tab:

<Image
  src={`${process.env.ASSETS}/docs/common/select-settings.png`}
  width={626 / 2}
  height={340 / 2}
  shadow
  borderRadius
/>
<Caption>Selecting the <b>Settings</b> tab from the Project Overview page.</Caption>

### Build & Development Settings and Root Directory

You can edit build and development settings as well as the root directory settings. Check out [our Build Step documentation](/docs/v2/build-step) to learm more.

### Logs and Source Access

If you turn on **Logs and Source Access**, the following paths for your project will be publicly accessible. Even if this setting is disabled, you can still access these paths by logging in.

- `/_src`: Displays the source code and build output.
- `/_logs`: Displays the build logs.

<Note type="warning">
  Do not edit this setting if you don’t want your source code or logs to be
  publicly accessible.
</Note>

#### Ony applied starting from your next deployment

As with other project settings, the changes you make to this setting will only be applied starting from your **next deployment**.

<Note>
  When you toggle this setting, none of your existing deployments will be
  affected. If you’d like to make the source code or logs private on your
  existing deployments, the only option is to delete them.
</Note>

### Custom Domains

You can add **custom domains** for each project. Check out [our Custom Domains documentation](/docs/v2/custom-domains) to learn more.

### Git Integration

To edit your project’s Git integration settings, select the **Git** menu item from your project settings page.

<Image
  src={`${process.env.ASSETS}/docs/advanced/deploy-hooks/select-git.png`}
  width={374 / 2}
  height={336 / 2}
  shadow
/>
<Caption>Selecting the <b>Git</b> menu item from the project settings page.</Caption>

If you haven’t set up your Git integration for [GitHub](/docs/v2/git-integrations/zeit-now-for-github), [GitLab](/docs/v2/git-integrations/zeit-now-for-gitlab), or [Bitbucket](/docs/v2/git-integrations/zeit-now-for-bitbucket) yet, you can do so from here.

<Image
  src={`${process.env.ASSETS}/docs/platform/projects/git-integration-not-connected.png`}
  width={1548 / 2}
  height={722 / 2}
  shadow
/>
<Caption>Setting up the Git integration.</Caption>

If you have already set up your Git Integration, you can specify which Git repository to connect your project to by entering the repository name.

<Image
  src={`${process.env.ASSETS}/docs/platform/projects/git-integration-connected.png`}
  width={1546 / 2}
  height={712 / 2}
  shadow
/>
<Caption>Specify the Git repository for your project.</Caption>

If you’d like to disconnect your project from a Git repository, **empty the field** and save it.

#### Disconnect Your Personal Account or Team

If you’d like to disconnect your **personal account** or **team** from GitHub, GitLab, or Bitbucket, follow the directions here:

- [Disconnect from GitHub](/docs/v2/git-integrations/zeit-now-for-github#disconnect-from-github)
- [Disconnect from GitLab](/docs/v2/git-integrations/zeit-now-for-gitlab#disconnect-from-gitlab)
- [Disconnect from Bitbucket](/docs/v2/git-integrations/zeit-now-for-bitbucket#disconnect-from-bitbucket)

### Deploy Hooks

ZEIT Now supports **deploy hooks**, which are unique URLs that accept HTTP POST requests and trigger deployments. Check out [our Deploy Hooks documentation](/docs/v2/more/deploy-hooks) to learn more.

## Deleting a Project

<Note type="warning">
  Deleting your project will also delete the deployments and domains within it.
  If you have any deployments that are assigned to a custom domain and do not
  want them to be removed, make sure to deploy and assign them to the custom
  domain under a different project first.
</Note>

To delete a project, choose a project from the [dashboard](https://zeit.co/dashboard):

<Image
  src={`${process.env.ASSETS}/docs/common/select-project.png`}
  width={1102 / 2}
  height={410 / 2}
  shadow
/>
<Caption>Selecting the project from the ZEIT Dashboard.</Caption>

Then, select the **Settings** tab:

<Image
  src={`${process.env.ASSETS}/docs/common/select-settings.png`}
  width={626 / 2}
  height={340 / 2}
  shadow
  borderRadius
/>
<Caption>Selecting the <b>Settings</b> tab from the Project Overview page.</Caption>

Next, select **Advanced** from the menu.

<Image
  src={`${process.env.ASSETS}/docs/platform/projects/advanced-menu.png`}
  width={394 / 2}
  height={338 / 2}
  shadow
  borderRadius
/>
<Caption>Selecting the <b>Advanced</b> menu item from the Project Settings page.</Caption>

You’ll see the **Delete Project** section. Confirm that you'd like to delete the project and click the **Delete** button.

<Image
  src={`${process.env.ASSETS}/docs/platform/projects/delete-project.png`}
  width={1562 / 2}
  height={628 / 2}
  shadow
  borderRadius
/>
<Caption>The <b>Delete Project</b> section.</Caption>

export default ({ children }) => <Doc meta={meta}>{children}</Doc>

export const config = {
  amp: 'hybrid'
}
