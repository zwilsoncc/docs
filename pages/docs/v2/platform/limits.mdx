...
The maximum amount of domains that can be applied to a single project, either from the <OrgName /> Dashboard or by using the `alias` key in a `now.json` file is **50**.

It is not possible to add more than 50 domains to a project using the <OrgName /> Dashboard.
