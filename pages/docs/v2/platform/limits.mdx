...
| Deployments created from CLI per week  | 25    | 25   | Custom     |
...
The maximum amount of domains that can be applied to a single project, either from the <ProductNameSimple /> Dashboard or by using the `alias` key in a `now.json` file is **50**.

It is not possible to add more than 50 domains to a project using the <ProductNameSimple /> Dashboard.
