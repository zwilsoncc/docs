...
export const meta = {
  ...
  lastEdited: '2020-02-22T08:13:16.000Z'
}
...
##### Question: What if I don’t need to build anything?

**Answer**: Some projects do not require building. An example of this would be a website with basic HTML/CSS/JS/assets that can be directly uploaded without using any packaging tools. You might just have a single `index.html` file.
...
##### Question: What if I just want to serve the files in the root directory?

**Answer:** You should’ve chosen "Other" as the framework preset. In that case, by default, the output directory will be set as `public` if it exists, or `.` (current directory) otherwise. So as long as it doesn’t have the `public` directory, it will serve the files in the root directory by default. Or you can toggle the **Override** setting and leave the field empty (in which case, the build step will be skipped).
...
<Note>
  ...
  (like <InlineCode>next dev</InlineCode> for Next.js).
</Note>

The Development Command setting allows you to customize the behavior of <InlineCode>now dev</InlineCode>.
