...
export const meta = {
  ...
  lastEdited: '2020-02-24T22:58:52.000Z'
}

When you deploy on ZEIT Now, we **build** your project. For many frontend frameworks, we automatically configure the build settings, but you can also [customize](#customizing-build-settings) them. You can also use [environment variables](#using-environment-variables-and-secrets) to avoid hardcoding values.

## Customizing Build Settings

To customize build settings for your project, choose a project from the [dashboard](https://zeit.co/dashboard):
...
You can then edit build settings from the **Build & Development Settings** and **Root Directory** sections.
...
If no framework is detected, it will be specified as "Other". You can choose a different framework preset or "Other" if you’d like.
...
- If `package.json` contains the `build` command, this command will be used to build the project.
- If not, `next build` will be the build command.

If you’d like to override the build command, you can toggle the **Override** setting on and specify the command.

<Note>
  If you update this setting, it will be applied starting from your next
  deployment.
</Note>

**If build is not required**: Some projects do not require building. An example of this would be a website with basic HTML/CSS/JS/assets that can be directly uploaded without using any packaging tools. You might just have a single `index.html` file.
...
Now, your project might not require building, and you might just want to serve the files directly under the root directory. In this case, you should choose "Other" as the framework preset.

If you do so, by default, the output directory will be set as `public` if it exists, or `.` (current directory) otherwise. Therefore, as long as your project doesn’t have the `public` directory, it will serve the files in the root directory by default. Or you can toggle the **Override** setting and leave the field empty (in which case, the build step will be skipped).
...
If you’d like to use a custom command for <InlineCode>now dev</InlineCode>, you can toggle the **Override** setting. Please note the following:

- If you specify a custom command, your command must pass the `$PORT` variable to your framework.
...
In some projects, the top-level directory of the repository may not be the root directory of the frontend app you’d like to build. For example, your repository might have a `frontend` directory, which contains a stand-alone [Next.js](https://nextjs.org/) app.

In cases like this, you can specify the project root directory here. If you do so, please note the following:

- If you specify a root directory here, then your app won’t be able to access files outside of that directory. You also cannot use `..` to move up a level.
- This works with [Now CLI](/download). Instead of doing `now <directory-name>` to deploy, specify `<directory-name>` here.
