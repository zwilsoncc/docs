...
Builders are modules that transform your source code into compiled outputs, being either static files and/or [Serverless Functions](https://zeit.co/docs/v2/deployments/concepts/lambdas/), which are served by our [CDN at the edge](https://zeit.co/docs/v2/advanced/domains/cdn/).
...
By default, the Static Builds Builder is used when ZEIT Now detects a `build` script inside a `package.json` file at the root of a project. This script is executed when deploying, and if the output is built to a `public` directory, that directory is served.
...
<TerminalInput>npm install @now/node --save-dev</TerminalInput>
...
The Go Builder is used by ZEIT Now to compile Go Serverless Functions that expose a single HTTP handler, including using the `net/http` Go API, from a `.go` file within an `/api` directory at your project's root.
...
Go version 1.x is used for Go projects deployed on Now.
...
The Python Builder is used by ZEIT Now to compile Python Serverless Functions, that defines a singlular HTTP `handler` variable, inheritting from the `BaseHTTPRequestHandler` class, from a `.py` file within an `/api` directory at your project's root.
...
Python projects deployed on ZEIT Now use Python version 3.6.
...
The Ruby Builder is used by ZEIT Now to compile Ruby Serverless Functions that define a singlular HTTP handler from `.rb` files within an `/api` directory at your project's root.
...
If your project has source files that need to be built requiring a different structure than ZEIT Now provides by default, you can use Builders with advanced configuration to achieve that structure.
...
Since Builders are published to [npm](https://npmjs.com), the `use` statement in your [`now.json` file](/docs/v2/deployments/configuration) uses a similar syntax to `npm install`, meaning that versioning can be used by tagging the builders, like for example:
...
To install private npm modules, define `NPM_TOKEN` as a [build environment variable](/docs/v2/deployments/configuration#build-env) in a `now.json` file.
...
Extending the feature-set of a Now deployment is as simple as creating a Builder that takes a list of files and outputs either static files or dynamic [Serverless Functions](/docs/v2/deployments/concepts/lambdas).
