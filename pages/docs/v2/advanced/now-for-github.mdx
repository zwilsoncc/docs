...
To stop Now from commenting on pull requests and commits, use the following configuration option in [a `now.json` file](/docs/v2/advanced/configuration) for the repository you want to stop this behavior in.
...
To stop Now from canceling in-progress builds to prioritise building newer commits, use the following configuration option in [a `now.json` file](/docs/v2/advanced/configuration) for the repository that this behavior should exist in.
