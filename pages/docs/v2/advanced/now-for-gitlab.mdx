...
You may want to use different workflows and APIs based on the environment of the deployment based on Git information. To support that, Now will deploy your app with the following built-in [environment variables](/docs/v2/serverless-functions/env-and-secrets) when deploying with Now for GitLab:
