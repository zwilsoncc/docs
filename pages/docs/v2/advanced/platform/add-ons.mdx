...
export const meta = {
  title: 'Add-Ons',
  description: 'Using ZEIT Now with additional features for more usecases.',
  editUrl: 'pages/docs/v2/advanced/platform/add-ons.mdx',
  lastEdited: '2019-08-22T10:47:21.000Z'
}

In the case that the default features of ZEIT Now are not enough to meet your needs, you can enable so-called "Add-Ons" on the dashboard.
...
<Image src={`https://assets.zeit.co/image/upload/v1541369048/front/docs/add-ons/add-ons.png`}
  ...
  height={1192/2}
/>
<Caption>The list of addons from the ZEIT Now <GenericLink href="/pricing">pricing page</GenericLink></Caption>
...
Below, you can find more details about what makes each Add-On unique and how those features can improve your experience of ZEIT Now.
...
By default, every deployment you create comes with a unique URL. This URL, by default, ends with `.now.sh` – a suffix that is shared among all deployments that are created on ZEIT Now.

If you do not want anybody to access your deployment, not passing the `--public` flag in [Now CLI](/download) is already a huge leap forward, as the code is private by default. In turn, nobody can access the source code of any deployment you create on ZEIT Now.
...
With the custom deployment suffix active, both the `scope` (by default, the account name), and the standard `.now.sh` suffix will be replaced by the custom deployment suffix in all deployments URL's.
