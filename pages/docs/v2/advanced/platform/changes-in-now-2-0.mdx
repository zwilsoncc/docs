...
- There are **no** **deployment types**. A single deployment can have multiple static and [dynamic entrypoints](/docs/v2/advanced/concepts/serverless-functins), even written in multiple different [languages](#migration-examples)
...
- Multiple **parallel builds** can be specified to compile source into [static files or Serverless Functions](/docs/v2/advanced/builders)
...
- Cost of logging (no more storage costs, charged [per-line](/docs/v2/advanced/concepts/serverless-functions#capabilities-and-limits) and [strictly](/docs/v2/advanced/concepts/serverless-functions#capabilities-and-limits) ephemeral)
...
- Faster cold boot-up times due to refined [entrypoint granularity](/docs/v2/advanced/concepts/serverless-functions#per-entrypoint-granularity)
...
While it's still very useful to think about what happens when an [instance of a container](/docs/v2/advanced/concepts/serverless-functions/#versus-processes-and-containers) is running or not as it pertains to hot vs cold invocation of [Serverless Functions](/docs/v2/advanced/concepts/serverless-Functions), attempts to count or enumerate instances in a UI are impractical and not very beneficial to the developer experience.

### No Config Mutability

All static assets and caches are served from our [Global CDN](/docs/v2/advanced/concepts/cdn-and-global-distribution) **by default**. When it comes to code invocation with [Serverless Functions](/docs/v2/advanced/concepts/serverless-functions), it makes sense to think about whether the code is executed in a single region or multiple.

We support the [`regions`](/docs/v2/advanced/configuration#regions) parameter in `now.json` to this end. When you make a deployment and Serverless Functions are produced in the [build steps](/docs/v2/build-step), we deploy them to the specified regions.

<Note hint>
  For the best Serverless Function performance, we recommend using the region{' '}
...
Furthermore, we have redesigned the directory listing style and added a new icon to represent [Serverless Functions](/docs/v2/advanced/concepts/serverless-functions):

<Image src={`https://assets.zeit.co/image/upload/v1541620549/front/docs/v2-upgrade/lambda-icon.png`}
width={1456/2}
height={962/2}
/>
<Caption>The new Lambda icon shows in the source view of deployments to show which files are Serverless Functions.</Caption>
...
- ~~`now scale`~~ is no longer necessary for 2.0 and deprecated. If you wish to change the regions your Serverless Functions get deployed to, use the [`regions`](/docs/v2/advanced/configuration#regions) config and re-deploy
