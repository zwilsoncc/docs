...
export const meta = {
  ...
  lastEdited: '2019-10-09T20:01:55.000Z'
}
...
We support the [`regions`](/docs/configuration#configuration/regions) parameter in `now.json` to this end. When you make a deployment and Serverless Functions are produced in the [build steps](/docs/v2/build-step), we deploy them to the specified regions.
...
- [`routes`](/docs/configuration#configuration/routes) to define routes and rewrites
...
- ~~`now scale`~~ is no longer necessary for 2.0 and deprecated. If you wish to change the regions your Serverless Functions get deployed to, use the [`regions`](/docs/configuration#configuration/regions) config and re-deploy
...
<Card title="Custom Domains" href="/docs/v2/custom-domains">
  Learn more about using custom domains with your deployments.
</Card>
