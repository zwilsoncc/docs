...
We support the [`regions`](/docs/configuration#project/regions) parameter in `now.json` to this end. When you make a deployment and Serverless Functions are produced in the [build steps](/docs/v2/build-step), we deploy them to the specified regions.
...
- [`routes`](/docs/configuration#project/routes) to define routes and rewrites
...
- ~~`now scale`~~ is no longer necessary for 2.0 and deprecated. If you wish to change the regions your Serverless Functions get deployed to, use the [`regions`](/docs/configuration#project/regions) config and re-deploy
