...
All static assets and caches are served from our [Global CDN](/docs/v2/deployments/concepts/cdn-and-global-distribution) **by default**. When it comes to code invocation with [Lambdas](/docs/v2/advanced/concepts/lambdas), it makes sense to think about whether the code is executed in a single region or multiple.
...
- ~~`now scale`~~ is no longer necessary for v2 and deprecated. If you wish to change the regions your Lambdas get deployed to, use the [`regions`](/docs/v2/advanced/configuration#regions) config and re-deploy
