...
You can deploy to the defined aliases using Now CLI by setting the [production deployment environment target](/docs/v2/custom-domains#deploying-with-your-domain).
...
<Note>
  Aliases from a `now.json` file will be **ignored** when there are domains that have been <GenericLink href="/docs/v2/custom-domains/#deploying-with-your-domain">configured for the project through the dashboard</GenericLink>.
</Note>
...
This value does not impact static files or edge caches, since deployments always have a [CDN layer](/docs/v2/advanced/domains/cdn) in front.
