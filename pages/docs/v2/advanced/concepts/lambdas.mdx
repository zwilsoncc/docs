...
Now automatically provisions Lambdas with the underlying [cloud infrastructure provider](/docs/v2/advanced/platform/regions-and-providers) for dynamic code execution (such as deployments that expose APIs), to run static site generators, or to compile code in the cloud.
