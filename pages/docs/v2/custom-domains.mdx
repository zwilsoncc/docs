...
This domain can be replaced with a **Custom Domain** of your choice. This Custom Domain can be purchased with <ProductNameSimple /> or a third-party.

In this document, adding a domain to your <ProductName /> [projects](/docs/v2/platform/projects/) is explained in detail.

## Adding a Domain

If you don't own a domain yet, you can [purchase it with <ProductNameSimple />](https://zeit.co/domains). Then, once you own a domain, adding it to a project is very straightforward:
...
You can move domains to another profile or team using the **Domains** tab from the **<ProductNameSimple /> Dashboard**.
...
<Image
  ...
  shadow
/>
<Caption>Selecting the <b>Domains</b> tab from the <ProductNameSimple /> Dashboard page.</Caption>
...
If the above `curl` command returns an error (e.g. `{"error":{...}}`), then you’re good to go - it means that <ProductNameSimple /> can generate certificates successfully. Otherwise, make sure that SSL is disabled for `/.well-known/**` and try again.
