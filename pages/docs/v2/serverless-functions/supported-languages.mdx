...
As an example; a file called `name.ts` in the `api` directory, and importing types for the ZEIT Now platform [Request and Response]() objects from [the `@now/node` module]():
