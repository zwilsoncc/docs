...
export const meta = {
  title: 'Supported Lanaguages for Serverless Functions',
  ...
  lastEdited: '2019-07-20T00:06:26.000Z'
}

Within the `api` directory of your projects, ZEIT Now will automatically recognise the languages listed on this page, through their file extensions, and serve them as serverless function.
...
For example, the following would live in `api/name.js`:
...
<Caption>
  An example Node.js function that recieves a <InlineCode>name</InlineCode>{' '}
  query and returns a greeting string.
</Caption>

Example deployment: <https://node-api.now-examples.now.sh/api/name?name=reader>
...
Deploying a Node.js function with the `.ts` extension will automatically be recognised as a TypeScript file and compiled to a serverless function.

As an example; a file called `name.ts` in the `api` directory, and importing types for the Now platform [Request and Response]() objects from [the `@now/node` module]():
...
<Caption>
  An example TypeScript Node.js function that recieves a{' '}
  <InlineCode>name</InlineCode> query and returns a greeting string.
</Caption>

Example deployment: <https://ts-api.now-examples.now.sh/api/name?name=reader>

You can install the `@now/node` module for type definitions through npm:

<TerminalInput>npm i -D @now/node</TerminalInput>
<Caption>Installing the <InlineCode>@now/node</InlineCode> module for Type defitions of <InlineCode>NowRequest</InlineCode> and <InlineCode>NowResponse</InlineCode></Caption>

You can also define a `tsconfig.json` to configure the Now TypeScript compiler:
...
Go files within the `api` directory, containing an a singular exported function integrated with the `net/http` Go API, will be served as a serverless function.
...
<Note>
  Your Go function must be an exported function that implements the{' '}
  <InlineCode>net/http</InlineCode> API.
</Note>

When deployed, the example function above will be served as a serverless function, returning the latest date. See it live with the following link: <https://go-api.now-examples.now.sh/api/name>
...
**Default Runtime Version**: PHP 7.1.x

PHP files within the `api` directory will be served as a serverless function.

For example, the following would go in `api/date.php`:

```php
<?php echo date(DATE_RFC2822); ?>
```

<Caption>An example PHP file that prints the current date.</Caption>

When deployed, the example PHP file above will be served as a serverless function, returning the current date and time. See it live with the following link: <https://php-api.now-examples.now.sh/api/date>

### PHP Global Variables

All standard PHP variables like `$_GET`, `$_POST` are supported, with the exception of `$_SESSION` and `$_FILES`.
...
>
  For more information on how to get started with Serverless Functions on Now
  and how you can develop them further, read the serverless functions
  introduction.
</Card>
