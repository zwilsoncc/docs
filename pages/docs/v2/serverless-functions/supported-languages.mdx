...
export const meta = {
  ...
  lastEdited: '2020-06-19T17:41:00.000Z'
}
...
As an example; a file called `hello.ts` in the `api` directory, and importing types for the <ProductName /> platform Request and Response objects from the `@vercel/node` module:
...
<Snippet dark text="npm i -D @vercel/node" />
<Caption>Installing the <InlineCode>@vercel/node</InlineCode> module for Type definitions of <InlineCode>NowRequest</InlineCode> and <InlineCode>NowResponse</InlineCode></Caption>
