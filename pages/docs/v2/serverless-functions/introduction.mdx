...
export const meta = {
  ...
  lastEdited: '2019-07-20T22:04:20.000Z'
}
...
For example, creating a `name` directory (within `/api`) with a `[name].js` file will allow you to recieve the path segment entered when accessing `/api/name/your-name`.
...
<Caption>
  An example of a Node.js serverless function that takes a name path segement
  and returns a string using it.
</Caption>
