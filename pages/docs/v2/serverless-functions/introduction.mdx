...
​<ProductName /> provides an additional command with [<ProductNameShort /> CLI](/download) to help you develop Serverless Functions locally by replicating the production environment on <ProductName /> with your localhost.
...
<Caption>Starting a local development environment using <ProductNameShort /> CLI, for Serverless Functions.</Caption>

<Note>
  The <InlineCode>now dev</InlineCode> command is still in beta. If you have any
  feedback, <Link href="https://zeit.co/feedback">please let us know</Link>.
</Note>
