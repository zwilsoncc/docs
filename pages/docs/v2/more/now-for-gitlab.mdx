...
The ZEIT Now for GitLab integration automatically deploys your GitLab projects with [ZEIT Now](/now), providing preview URLs and automatic production domain updates.

ZEIT Now for GitLab provides:
...
## Set up ZEIT Now for GitLab

### Step 1: Link ZEIT ZEIT Now to Your GitLab Account

You can connect your ZEIT Now account to GitLab by visiting your [account or team](/docs/v2/platform/users-and-teams) [settings page](/account) and clicking the "Connect with GitLab" button.
...
height={384/2}
/>
<Caption>The ZEIT Now for GitLab installation section within the account or team general settings page.</Caption>

Next, you will need to authorize the ZEIT Now application on GitLab with your account.
...
Alternatively; if you do not already have a ZEIT Now account, you can sign up to ZEIT using your GitLab account from [the sign up page](/signup).
...
With each new push, if ZEIT Now is already building a previous commit, it will cancel that current build to start the most recent commit, ensuring you always have the latest changes deployed as quickly as possible.
...
You may want to use different workflows and APIs based on the environment of the deployment based on Git information. To support that, ZEIT Now will deploy your app with the following built-in [environment variables](/docs/v2/serverless-functions/env-and-secrets) when deploying with ZEIT Now for GitLab:
