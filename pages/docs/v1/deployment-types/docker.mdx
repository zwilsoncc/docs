...
export const meta = {
  ...
  description: `Docker apps with ${PRODUCT_SHORT_NAME_V1} and all about the deployments`,
  date: '09 March 2017',
...
​<ProductShortNameV1 />'s Dockerfile support represents a significant departure from traditional virtualization solution offered by mainstream cloud providers. Here's why:
...
In the equivalent amount of time, I would be able to push and build the same image to <ProductShortNameV1 /> **18 times**. The time that's not spent building is spent on iterating on your product and sharing it with the world.
...
By default, <ProductShortNameV1 /> will build an image of your project by itself. If you want to speed up this process, you can publish a pre-built image on the public [Docker Hub](https://hub.docker.com/). At the moment, we don't support private images. However, it's on our roadmap!
...
​<ProductShortNameV1 /> deployment instances always listen on port `443` (HTTPS) of their given URL. Your server code can expose an HTTP service on **any port** of your choice (not multiple ones). <ProductShortNameV1 /> will then route requests received on port `443` to your HTTP service port and its response will then be returned by the deployment instance.
