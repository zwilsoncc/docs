...
This command will remove all your non-aliased deployments with the name `my-app`. This can be run after the `vercel alias` command to remove the previous deployment of the project.
...
Deployments are immutable. This means they cannot be modified after being created. To update your application, you need to deploy the new version using the command `vercel`. After that, you will get a new unique deployment URL similar to `my-app-hjrehxuuih.now.sh`.
...
- **Staging and production deployments**<br />
  A new deployment gives you a unique URL you can use as staging to share with co-workers or clients. After it is confirmed that it works, you can upgrade it to production with a single command: `vercel alias`.
...
1. From the current owner of the domain: `vercel domain rm domain.com`
2. From the desired owner of the domain: `vercel domain add domain.com`
...
This behavior can be completely avoided using the `vercel scale` command as described in the [previous link](/docs/v1/getting-started/scaling#default-scaling).
...
​<ProductShortV1Name /> doesn't provide you with an email server for your custom domains. You can use [`vercel dns`](/docs/features/dns) to setup MX records pointing to any external service.
...
Yes, you can use `vercel alias` to move an existing alias from a deployment to another the same way you use it to assign it the first time.
...
Due to the [immutable nature of deployments](/docs/other/faq#how-do-i-update-my-deployment's-files-or-code), each time you run `vercel` you will get a new unique URL. To have a custom static URL you need to use the `vercel alias` command.
