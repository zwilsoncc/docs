...
​<ProductNameShortV1 /> will deploy to the nearest region available automatically by default. You can override this behavior by either:
...
- **Zero-Downtime deployment**<br />
  Most services will require you to restart your server (eg. Node.js). Thanks to <ProductNameShortV1 />, you get a new deployment. You can wait until the deployment is ready to change the alias and avoid any downtime during the deployment process.

## Can I run a database on the <ProductNameWrapper name={PRODUCT_SHORT_NAME_V1} /> platform?

​<ProductNameShortV1 /> deployments **must** expose a [single](/docs/deployment-types/node#port-selection) [port](/docs/deployment-types/docker#port-selection) running an HTTP or WebSocket server. With Docker, however, it is possible to run a database in the same container of the HTTP API consuming it.
...
We recommend that you run your database on a database hosting service such as [Cosmos DB](https://azure.microsoft.com/en-gb/services/cosmos-db/), [Redis Labs](https://redislabs.com/), [MongoDB Atlas](https://www.mongodb.com/cloud/atlas), or [Compose](https://www.compose.com/). With these services, you will be able to match the regional locations available in <ProductNameShortV1 /> and scale individually to provide equal service to your database than with your app deployments.
...
For now, it is not possible to transfer a domain into <OrgNameV1 /> Domains, but you can [change your domain nameservers to point to zeit.world](https://zeit.co/world) to use <ProductNameShortV1 /> as a DNS.
...
At the moment <ProductNameShortV1 /> does not support docker-compose. Keep an eye on the [open issue on GitHub](https://github.com/zeit/now/issues/294) to know when it will be supported.

## How do I disable HTTPS and just use HTTP Instead?

There is no way to disable SSL. All <ProductNameShortV1 /> deployments are HTTPS by default.
...
Check out our [Setting up a Redirect with <ProductNameShortV1 />](/docs/guides/redirect) guide for instructions.
...
The IP addresses of <ProductNameShortV1 /> deployments are too dynamic and for that reason, we don't provide a list of them.
...
​<ProductNameShortV1 /> doesn't provide you with an email server for your custom domains. You can use [`now dns`](/docs/features/dns) to setup MX records pointing to any external service.
...
If you do not want to renew a domain purchased with <ProductNameShortV1 /> Domains, remove it with the following command:
...
We block ports 25 and 587 on <ProductNameShortV1 /> deployments to avoid spam.
