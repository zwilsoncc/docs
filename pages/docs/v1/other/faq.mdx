...
This command will remove all your non-aliased deployments with the name `my-app`. This can be run after the `vc alias` command to remove the previous deployment of the project.
...
Deployments are immutable. This means they cannot be modified after being created. To update your application, you need to deploy the new version using the command `vc`. After that, you will get a new unique deployment URL similar to `my-app-hjrehxuuih.now.sh`.
...
- **Staging and production deployments**<br />
  A new deployment gives you a unique URL you can use as staging to share with co-workers or clients. After it is confirmed that it works, you can upgrade it to production with a single command: `vc alias`.
...
1. From the current owner of the domain: `vc domain rm domain.com`
2. From the desired owner of the domain: `vc domain add domain.com`
...
This behavior can be completely avoided using the `vc scale` command as described in the [previous link](/docs/v1/getting-started/scaling#default-scaling).
...
​<ProductShortV1Name /> doesn't provide you with an email server for your custom domains. You can use [`vc dns`](/docs/features/dns) to setup MX records pointing to any external service.
...
Yes, you can use `vc alias` to move an existing alias from a deployment to another the same way you use it to assign it the first time.
...
Due to the [immutable nature of deployments](/docs/other/faq#how-do-i-update-my-deployment's-files-or-code), each time you run `vc` you will get a new unique URL. To have a custom static URL you need to use the `vc alias` command.
