...
import ProductShortNameV1 from '~/components/product-name/short-v1'
import { TerminalInput } from '~/components/text/terminal'
import OrgNameV1 from '~/components/product-name/org-name-v1'
import ProductNameWrapper from '~/components/product-name/wrapper'
import { PRODUCT_SHORT_NAME_V1, ORGANIZATION } from '~/lib/constants'
import CliNameV1 from '~/components/product-name/cli-name-v1'

export const meta = {
  title: 'Frequently Asked Questions',
  description: `The answers to our most frequently asked questions for ${PRODUCT_SHORT_NAME_V1} and other ${ORGANIZATION} products and services`,
  date: '30 Jan 2018',
  editUrl: 'pages/docs/v1/other/faq.mdx',
  lastEdited: '2020-04-13T20:57:18.000Z'
}
...
Whenever you upload files using <CliNameV1 />, a new deployment is
...
​<ProductShortNameV1 /> will deploy to the nearest region available automatically by default. You can override this behavior by either:
...
- **Zero-Downtime deployment**<br />
  Most services will require you to restart your server (eg. Node.js). Thanks to <ProductShortNameV1 />, you get a new deployment. You can wait until the deployment is ready to change the alias and avoid any downtime during the deployment process.

## Can I run a database on the <ProductNameWrapper name={PRODUCT_SHORT_NAME_V1} /> platform?

​<ProductShortNameV1 /> deployments **must** expose a [single](/docs/deployment-types/node#port-selection) [port](/docs/deployment-types/docker#port-selection) running an HTTP or WebSocket server. With Docker, however, it is possible to run a database in the same container of the HTTP API consuming it.
...
We recommend that you run your database on a database hosting service such as [Cosmos DB](https://azure.microsoft.com/en-gb/services/cosmos-db/), [Redis Labs](https://redislabs.com/), [MongoDB Atlas](https://www.mongodb.com/cloud/atlas), or [Compose](https://www.compose.com/). With these services, you will be able to match the regional locations available in <ProductShortNameV1 /> and scale individually to provide equal service to your database than with your app deployments.

## Can I transfer domains into/out of <ProductNameWrapper name={ORGANIZATION} /> Domains?

We are working on a domain transfer tool to make this process easier.

For now, it is not possible to transfer a domain into <OrgNameV1 /> Domains, but you can [change your domain nameservers to point to zeit.world](https://zeit.co/world) to use <ProductShortNameV1 /> as a DNS.

If you need to transfer your domain out of <OrgNameV1 /> Domains, you can contact us at [support@zeit.co](mailto:support@zeit.co?subject=Domain%20Transfer) with the domain you would like to transfer.

If you would like to move a domain from one <OrgNameV1 /> account to another, you could do so by running the following commands:
...
After you have done this, the desired owner will be in possession of the domain and will be able to utilize it with <OrgNameV1 />.

## Can I use docker-compose with <ProductNameWrapper name={PRODUCT_SHORT_NAME_V1} />?

At the moment <ProductShortNameV1 /> does not support docker-compose. Keep an eye on the [open issue on GitHub](https://github.com/zeit/now/issues/294) to know when it will be supported.

## How do I disable HTTPS and just use HTTP Instead?

There is no way to disable SSL. All <ProductShortNameV1 /> deployments are HTTPS by default.
...
Check out our [Setting up a Redirect with <ProductShortNameV1 />](/docs/guides/redirect) guide for instructions.

## Can I run a bot on <ProductNameWrapper name={PRODUCT_SHORT_NAME_V1} />?
...
## How do I Change the nameservers of a domain purchased with <ProductNameWrapper name={ORGANIZATION} /> Domains?

This is not currently possible to do via the [<CliNameV1 />](/docs/clients/now-cli), but you can contact us at [support@zeit.co](mailto:support@zeit.co?subject=Change%20Purchased%20Domain%20Nameserver) with your desired nameservers and, after a security verification of the ownership, we can change them for you.
...
## Is It possible to host WordPress on the <ProductNameWrapper name={PRODUCT_SHORT_NAME_V1} /> Platform?

Check our WordPress example repository.

> <https://github.com/now-examples/wordpress>

## Is it possible to host Ghost on the <ProductNameWrapper name={PRODUCT_SHORT_NAME_V1} /> Platform?
...
The IP addresses of <ProductShortNameV1 /> deployments are too dynamic and for that reason, we don't provide a list of them.

Our recommendation when connecting to external services (eg. a database) is to use a strong password and SSL.

## How do I set up an email for a domain purchased through <ProductNameWrapper name={PRODUCT_SHORT_NAME_V1} /> Domains?

​<ProductShortNameV1 /> doesn't provide you with an email server for your custom domains. You can use [`now dns`](/docs/features/dns) to setup MX records pointing to any external service.
...
## Does <ProductNameWrapper name={PRODUCT_SHORT_NAME_V1} /> support wildcard subdomains?
...
If you do not want to renew a domain purchased with <ProductShortNameV1 /> Domains, remove it with the following command:
...
## How do I run scheduled tasks on the <ProductNameWrapper name={PRODUCT_SHORT_NAME_V1} /> platform?
...
## Is it possible to upload files to a running <ProductNameWrapper name={PRODUCT_SHORT_NAME_V1} /> instance?
...
## How do I set up a password for my <ProductNameWrapper name={ORGANIZATION} /> Account?

<OrgNameV1 />'s login is serverless. You just need your email address and don't need to remember yet another password.
...
## Does <ProductNameWrapper name={PRODUCT_SHORT_NAME_V1} /> offer support for IPv6?

We do not currently offer support for IPv6.

## Does <ProductNameWrapper name={PRODUCT_SHORT_NAME_V1} /> block outgoing connections in any way?

We block ports 25 and 587 on <ProductShortNameV1 /> deployments to avoid spam.
Any other port is available for use.

## Can I Uue a 3-D secure credit card with <ProductNameWrapper name={ORGANIZATION} />?

We do not currently offer support for the 3-D Secure protocol.

## Is <ProductNameWrapper name={PRODUCT_SHORT_NAME_V1} /> GDPR compliant?
...
## Why does <ProductNameWrapper name={PRODUCT_SHORT_NAME_V1} /> give me random URLs and how can I change it?
...
## How can I use a separate <ProductNameWrapper name={PRODUCT_SHORT_NAME_V1} /> configuration for staging and production?
