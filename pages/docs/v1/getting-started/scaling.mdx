...
Here, `hello-node-lwbxweoqjo.now.sh` is one of our deployment URL. Instead, you can scale a "domain name" you mapped with `vercel alias`.
...
If you run `vercel alias` again, it'll scale the new deployment accordingly and scale down the old deployment to the <Link href="/docs/getting-started/scaling#default-scaling">default scaling setup</Link>.
...
So, you don't need to invoke `vercel scale` for static deployments.
