...
export const meta = {
  title: 'Secrets',
  description: `Configuring your apps with environment variables that shouldn't be public using Secrets`,
  date: '6 August 2017',
...
That's where Secrets can help you. It's a configuration store that works across your account. Let's see how to use it:

First, add some secrets:

<TerminalInput>
  {`vercel secrets add my-app-mongo-url "user:password@mydb.com"
...
You can perform a few sets of operations with Secrets, including adding, renaming and removing secrets. But you can't read secrets from the terminal.

> **WARNING**:
>
> Anyone who can deploy to <ProductShortV1Name /> has access to these secrets. Disabling the ability to read secrets in the terminal is simply a barrier.
> A user can still deploy a simple app to dump these secrets.

You can get more information about Secrets by running the following Help command:
...
<Image
  ...
  width={650}
  height={509}
  caption="Help output of `vercel secrets`"
/>

## Secrets with New Lines

Sometimes, you need to add secrets which have newlines (or any other special characters) in them (eg: certificates). But you won't be able to add them by simply using `vercel secrets add`.
