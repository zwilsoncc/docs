...
That's where <ProductShortNameV1 /> Secrets can help you. It's a configuration store that works across your account. Let's see how to use it:
...
You can perform a few sets of operations with <ProductShortNameV1 /> Secrets, including adding, renaming and removing secrets. But you can't read secrets from the terminal.

> **WARNING**:
>
> Anyone who can deploy to <ProductShortNameV1 /> has access to these secrets. Disabling the ability to read secrets in the terminal is simply a barrier.
> A user can still deploy a simple app to dump these secrets.

You can get more information about <ProductShortNameV1 /> Secrets by running the following Help command:
