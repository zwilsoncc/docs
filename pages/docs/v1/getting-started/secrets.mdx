...
export const meta = {
  title: 'Secrets',
  description: `Configuring your apps with environment variables that shouldn't be public using ${PRODUCT_SHORT_V1} Secrets`,
  date: '6 August 2017',
  editUrl: 'pages/docs/v1/getting-started/secrets.mdx',
  lastEdited: '2020-04-13T07:12:12.000Z'
}

If multiple people deploy your app or utilize a CI service, it's a better idea to use <Link href="/docs/getting-started/environment-variables#via-“now.json”">now.json</Link> to expose environment variables.
...
## <ProductNameWrapper name={PRODUCT_SHORT_V1} /> Secrets

That's where <ProductNameShortV1 /> Secrets can help you. It's a configuration store that works across your account. Let's see how to use it:
...
You can perform a few sets of operations with <ProductNameShortV1 /> Secrets, including adding, renaming and removing secrets. But you can't read secrets from the terminal.

> **WARNING**:
>
> Anyone who can deploy to <ProductNameShortV1 /> has access to these secrets. Disabling the ability to read secrets in the terminal is simply a barrier.
> A user can still deploy a simple app to dump these secrets.

You can get more information about <ProductNameShortV1 /> Secrets by running the following Help command:

<TerminalInput>now secrets --help</TerminalInput>

<Image
  src={`${process.env.IMAGE_ASSETS_URL}/docs/now-secrets/help.png`}
  width={650}
  height={509}
