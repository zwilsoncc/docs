...
export const meta = {
  title: 'Deployment',
  description: `How to setup and deploy Node.js apps, Docker apps, or static websites with ${PRODUCT_SHORT_NAME_V1}`,
  date: '1 August 2017',
  editUrl: 'pages/docs/v1/getting-started/deployment.mdx',
  lastEdited: '2020-04-13T20:57:18.000Z'
}

With <ProductShortNameV1 />, you can deploy any kind of web app by using a single command. <ProductShortNameV1 /> supports three types of deployments:
...
With static deployment, you can deploy a static web app or a set of assets to <ProductShortNameV1 />. Visit the directory you want to deploy and run this command:

<TerminalInput>now</TerminalInput>

If that directory contains an `index.html` file, that file will be served. Otherwise, <ProductShortNameV1 /> will show all the files in that directory.
...
If you have a `package.json` file in your app directory, <ProductShortNameV1 /> considers that a valid Node.js deployment. Here is a simple Node.js deployment with the help of [micro](https://github.com/zeit/micro).
...
​<ProductShortNameV1 /> will install dependencies and run the `start` NPM script, as mentioned in the above `package.json` file.
...
If your app directory contains a `Dockerfile`, <ProductShortNameV1 /> considers that a valid [Docker](https://www.docker.com/) deployment. It will build a docker image based on the `Dockerfile` and start container(s) based on that.
...
<Note>
  Without enabling <ProductShortNameV1 /> Cloud v2 with the following{' '}
...
<Caption>
  Enabling <ProductShortNameV1 /> Cloud v2 from a `now.json` configuration file.
</Caption>
...
To learn more about Docker deployments on <ProductShortNameV1 />, read <Link href="/docs/deployment-types/docker">this guide</Link>.
