...
Accessing logs via the `vercel` CLI is simple as invoking this command:
...
For more information on `vercel logs`, run the help command with:
...
Here's an advanced `vercel logs` command to let you inspect the last 10 HTTP GET requests.
