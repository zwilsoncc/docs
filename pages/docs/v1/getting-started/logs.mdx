...
Accessing logs via the `vc` CLI is simple as invoking this command:
...
For more information on `vc logs`, run the help command with:
...
Here's an advanced `vc logs` command to let you inspect the last 10 HTTP GET requests.
