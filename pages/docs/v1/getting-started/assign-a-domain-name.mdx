...
export const meta = {
  ...
  description: `Using ${PRODUCT_SHORT_V1_NAME} \`alias\` to assign a domain to your deployments`,
  date: '2 August 2017',
  editUrl: 'pages/docs/v1/getting-started/assign-a-domain-name.mdx',
  lastEdited: '2020-05-08T23:58:00.000Z'
}

When you deploy an app with <ProductShortV1Name />, you will have a unique deployment URL like this: <https://my-web-app-avvuiuuwto.now.sh>.
...
<Image
  ...
  }/docs/assign-a-domain-name/custom-domain.png`}
  width={600}
  height={325}
  caption="Our sample static app with a fixed domain name."
/>

In this guide, we are going to map a domain name into one of those deployment URLs of your app by using the `vercel alias` command.

The four different ways to use the `vercel alias` command are:

1. <Link href="#1.-using-a-now.sh-domain">Using a now.sh domain</Link>
2. <Link href="#2.-using-a-custom-domain,-managed-by-now">
     Using a custom domain, managed by now
   </Link>
3. <Link href="#3.-using-a-custom-domain,-dns-managed-by-now">
     Using a custom domain, DNS managed by now
   </Link>
4. <Link href="#4.-using-a-custom-domain-with-a-cname">
     Using a custom domain with a CNAME
   </Link>
...
If you want to use a custom domain for your app, the easiest method is to buy a domain name for your app using <ProductShortV1Name />.
...
After the confirmation, <ProductShortV1Name /> will handle all the technical details for you and allow your users to access your app using <https://my-web-app.com>.

<Image
  src={`${
    process.env.IMAGE_ASSETS_URL
  }/docs/assign-a-domain-name/buy-domain.png`}
  width={650}
  height={412}
  caption="Confirmation you'll get after assigning a custom domain."
/>

> In this process, <ProductShortV1Name /> will set up a [Let's Encrypt](https://letsencrypt.org/) SSL certificate for your domain automatically.

## 3. Using a custom domain, DNS managed by now

If you already have a domain name, this is the best option for you. With this method, <ProductShortV1Name /> will handle all the DNS management for your domain.

### Setup

First of all, you need to point your domain's nameservers to our [global DNS service](/world). Here is how to do that:

- Visit your domain name provider and find a place where you can add custom nameservers.
- Select two or more nameservers from [this list](/world#get-started).
- Set them as custom nameservers for your domain.

For example, assume that your domain name is `my-custom-domain.com`.

After you have set up the nameservers, simply run the following `vercel alias` command:
...
> Even in this process, <ProductShortV1Name /> will set up a [Let's Encrypt](https://letsencrypt.org/) SSL certificate for your domain automatically.
...
<Image
  src={`${
    process.env.IMAGE_ASSETS_URL
  }/docs/assign-a-domain-name/alias-error.png`}
  width={650}
  height={412}
  caption="The error message you'll get when trying to alias a domain for the first time."
/>

​<ProductShortV1Name /> wants to verify the ownership of your domain. That's why it throws an error.

- Next, add the TXT record as mentioned above error message.
- Run the `vercel alias` command again:
...
> In this case, <ProductShortV1Name /> will setup a [Let's Encrypt](https://letsencrypt.org/) SSL certificate for your domain. However, before you do that, you need to verify the domain.
