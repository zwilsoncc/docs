...
In this guide, we are going to map a domain name into one of those deployment URLs of your app by using the `vc alias` command.

The four different ways to use the `vc alias` command are:
...
After you have set up the nameservers, simply run the following `vc alias` command:
...
- Run the `vc alias` command again:
