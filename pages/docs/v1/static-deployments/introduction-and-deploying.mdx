...
Deployments stay around forever if they are not removed using [the `vercel remove` command](/docs/clients/now-cli#cloud-commands).
...
Static deployments running on <ProductShortV1Name /> have the same rules as [serve-handler](https://github.com/zeit/serve-handler), which is available and open to download, fork, extend, and operate locally during development by importing the module itself into an [existing server](https://github.com/zeit/serve#api) or
