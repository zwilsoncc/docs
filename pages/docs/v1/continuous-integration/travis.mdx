...
- Optionally; build an application inside Travis instead of with <ProductShortNameV1 />
...
With this token you can use <ProductShortNameV1 /> with your account anywhere with the following command:
...
First of all, make sure you've signed up with Travis and synced the repositories you'd like to use with <ProductShortNameV1 /> previous to this. If you have yet to do this, read steps 1 and 2 of the Travis CI's ["To get started with Travis CI" guide](https://docs.travis-ci.com/user/getting-started). If you're configuring a public repository, you can use [travis-ci.org](https://travis-ci.org), otherwise, if the repository is private, you'll need to use the paid version on [travis-ci.com](https://travis-ci.com).
...
<Note>
  ...
  <InlineCode>--public</InlineCode> argument with the <ProductShortNameV1 />{' '}
...
In some cases, you may prefer to build on Travis CI instead of directly on <ProductShortNameV1 />. If that's the case, this can be achieved by modifying your `.travis.yml` and `now.json` files a little bit.
...
With this, <ProductShortNameV1 /> will only deploy the files under the directories `.next`, `server`, `static`, and the files `package.json` and `package-lock.json`. You can customize this to add any files which will be required by your application after the build step.
...
Since you are building your application on Travis, you don't want <ProductShortNameV1 /> to try to build it. To avoid this you can add the following simple script to your `package.json`.
...
If the `now-build` script is defined, <ProductShortNameV1 /> will use it instead of `build`. This lets you define it yourself in order to customize the build step on <ProductShortNameV1 /> or just avoid building it on <ProductShortNameV1 /> entirely.
