...
export const meta = {
  ...
  description:
    'Deploy your changes in your GitHub repositories after every push using Travis CI and Now',
  date: '13 Mar 2018',
...
<Note>
  ...
  <InlineCode>--public</InlineCode> argument with the Now commands to avoid
  being asked if you are sure you want your deployment to be public, which will
  block the build; awaiting user input.
</Note>
...
With this, Now will only deploy the files under the directories `.next`, `server`, `static`, and the files `package.json` and `package-lock.json`. You can customize this to add any files which will be required by your application after the build step.
