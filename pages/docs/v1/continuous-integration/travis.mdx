...
export const meta = {
  ...
  description: `Deploy your changes in your GitHub repositories after every push using Travis CI and ${PRODUCT_SHORT_V1}`,
  date: '13 Mar 2018',
...
- Optionally; build an application inside Travis instead of with <ProductNameShortV1 />

## Step 1: Getting a <ProductNameWrapper name={PRODUCT_SHORT_V1} /> Token
...
With this token you can use <ProductNameShortV1 /> with your account anywhere with the following command:
...
First of all, make sure you've signed up with Travis and synced the repositories you'd like to use with <ProductNameShortV1 /> previous to this. If you have yet to do this, read steps 1 and 2 of the Travis CI's ["To get started with Travis CI" guide](https://docs.travis-ci.com/user/getting-started). If you're configuring a public repository, you can use [travis-ci.org](https://travis-ci.org), otherwise, if the repository is private, you'll need to use the paid version on [travis-ci.com](https://travis-ci.com).
...
## Step 3: Configuring Travis to deploy with <ProductNameWrapper name={PRODUCT_SHORT_V1} /> using your Token
...
<Note>
  ...
  <InlineCode>--public</InlineCode> argument with the <ProductNameShortV1 />{' '}
...
## Optional: Avoid building on <ProductNameWrapper name={PRODUCT_SHORT_V1} /> (Instead build on Travis)

In some cases, you may prefer to build on Travis CI instead of directly on <ProductNameShortV1 />. If that's the case, this can be achieved by modifying your `.travis.yml` and `now.json` files a little bit.
...
With this, <ProductNameShortV1 /> will only deploy the files under the directories `.next`, `server`, `static`, and the files `package.json` and `package-lock.json`. You can customize this to add any files which will be required by your application after the build step.
...
Since you are building your application on Travis, you don't want <ProductNameShortV1 /> to try to build it. To avoid this you can add the following simple script to your `package.json`.
...
If the `now-build` script is defined, <ProductNameShortV1 /> will use it instead of `build`. This lets you define it yourself in order to customize the build step on <ProductNameShortV1 /> or just avoid building it on <ProductNameShortV1 /> entirely.
