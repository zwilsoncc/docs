...
export const meta = {
  title: `The ${PRODUCT_SHORT_V1} CDN`,
  description: `Full-featured, flexible, automatic caching for ${PRODUCT_SHORT_V1} deployments, aliases, and domains.`,
  date: '10 July 2018',
...
<Note>
  The <ProductNameShortV1 /> CDN is now simplified and automatic. Please see the{' '}
...
​<ProductNameShortV1 /> CDN enabled domains are automatically fronted by over 150 locations, allowing you to immediately optimize all of your <ProductNameShortV1 /> deployments with a single command.
...
For any existing domains, added before the CDN was released, or domains that have had the CDN disabled, you can enable the CDN with the <CliNameV1 /> by running the following command:
...
When aliasing to any <ProductNameShortV1 /> CDN enabled domain or `now.sh` alias, we automatically purge the cache so that no stale content is ever delivered.
...
If no `Cache-Control` response header is present, the <ProductNameShortV1 /> routing layer adds `Cache-Control: s-maxage=0`, to ensure that no shared caches will cache content that wasn't explicitly marked as cacheable.
...
Currently, the <ProductNameShortV1 /> CDN can only be enabled for single level of subdomains. This means that, for example, `*.zeit.university` will be fronted by the CDN, while `*.dev.zeit.university` will be routed directly to our origin locations.
