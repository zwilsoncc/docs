...
<Note>
  The <ProductShortNameV1 /> CDN is now simplified and automatic. Please see the{' '}
...
​<ProductShortNameV1 /> CDN enabled domains are automatically fronted by over 150 locations, allowing you to immediately optimize all of your <ProductShortNameV1 /> deployments with a single command.
...
When aliasing to any <ProductShortNameV1 /> CDN enabled domain or `now.sh` alias, we automatically purge the cache so that no stale content is ever delivered.
...
If no `Cache-Control` response header is present, the <ProductShortNameV1 /> routing layer adds `Cache-Control: s-maxage=0`, to ensure that no shared caches will cache content that wasn't explicitly marked as cacheable.
...
Currently, the <ProductShortNameV1 /> CDN can only be enabled for single level of subdomains. This means that, for example, `*.zeit.university` will be fronted by the CDN, while `*.dev.zeit.university` will be routed directly to our origin locations.
