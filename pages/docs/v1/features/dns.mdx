...
Thanks to the `vc dns` CLI command, managing existing DNS records with <ProductShortV1Name /> is simple.
...
In the examples shown below, the placeholder `example.com` represents a domain of your choice that was registered with `vc domains` or `vc alias`. Here are the main sub-commands you can use:
...
This is the syntax for the `vc dns` command.
...
>
> 1. use a custom certificate for your <ProductShortV1Name /> deployment with <Link href="/docs/features/certs#now-certs-replace">`vc certs replace`</Link>
> 2. run an app outside of <ProductShortV1Name /> with TLS for a subdomain

### now dns rm &lt;ID>

Removes a record by ID, which is shown in the `vc dns ls` listing. Note that it may take a couple of hours before the change is fully propagated across our infrastructure.
