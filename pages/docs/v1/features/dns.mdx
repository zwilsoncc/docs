...
export const meta = {
  ...
  description: `Managing your DNS records for domains used with ${PRODUCT_SHORT_NAME_V1}`,
  date: '15 Mar 2017',
  editUrl: 'pages/docs/v1/features/dns.mdx',
  lastEdited: '2020-04-13T20:57:18.000Z'
}

Thanks to the `now dns` CLI command, managing existing DNS records with <ProductShortNameV1 /> is simple.

> However, this only applies to domains you bought via <ProductShortNameV1 /> or domains configured with our [global DNS service](/world).
...
>
> 1. use a custom certificate for your <ProductShortNameV1 /> deployment with <Link href="/docs/features/certs#now-certs-replace">`now certs replace`</Link>
> 2. run an app outside of <ProductShortNameV1 /> with TLS for a subdomain
