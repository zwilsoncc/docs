...
export const meta = {
  ...
  description: `Scaling your deployments to be instances in multiple global locations with ${PRODUCT_SHORT_V1}`,
  date: '01 Apr 2018',
...
is available in the region that you would like to enable on <ProductNameShortV1 />**.
...
When scaling deployments on <ProductNameShortV1 />, it is necessary
...
tell <ProductNameShortV1 /> where exactly it should scale
your deployment to.

​<ProductNameShortV1 /> provides a number of regions, each of which
consists of one or more [data centers](#data-centers).

​​<ProductNameV1 /> runs in two regions:
...
pass the `--regions` option to <CliNameV1 />:
...
In order to let <ProductNameShortV1 /> scale the deployment
...
let <ProductNameShortV1 /> scale your deployment automatically
