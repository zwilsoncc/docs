...
However, static content is cached in all edge regions. You can find the full list of regions in the [regions and providers documentation](/docs/v2/edge-network/regions/).
...
applied when `vercel` is run. In turn,
...
<TerminalInput>{`vercel scale <deployment> 4`}</TerminalInput>
...
<TerminalInput>{`vercel scale <deployment> 4 7`}</TerminalInput>
...
<TerminalInput>{`vercel scale <deployment> auto`}</TerminalInput>
...
<TerminalInput>{`vercel scale <deployment> 1 auto`}</TerminalInput>

### Enabling Additional Locations after Deploying

Using `vercel scale`, you can
...
<TerminalInput>{`vercel scale <deployment> sfo`}</TerminalInput>

Alternatively, you can also scale to all available [regions](#regions)...

<TerminalInput>{`vercel scale <deployment> all`}</TerminalInput>

...or specify multiple:

<TerminalInput>{`vercel scale <deployment> sfo,bru`}</TerminalInput>
...
<TerminalInput>{`vercel scale <deployment> <regions|datacenters> 1 5`}</TerminalInput>
