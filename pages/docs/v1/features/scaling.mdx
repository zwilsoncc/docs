...
applied when `vc` is run. In turn,
...
<TerminalInput>{`vc scale <deployment> 4`}</TerminalInput>
...
<TerminalInput>{`vc scale <deployment> 4 7`}</TerminalInput>
...
<TerminalInput>{`vc scale <deployment> auto`}</TerminalInput>
...
<TerminalInput>{`vc scale <deployment> 1 auto`}</TerminalInput>

### Enabling Additional Locations after Deploying

Using `vc scale`, you can
...
<TerminalInput>{`vc scale <deployment> sfo`}</TerminalInput>

Alternatively, you can also scale to all available [regions](#regions)...

<TerminalInput>{`vc scale <deployment> all`}</TerminalInput>

...or specify multiple:

<TerminalInput>{`vc scale <deployment> sfo,bru`}</TerminalInput>
...
<TerminalInput>{`vc scale <deployment> <regions|datacenters> 1 5`}</TerminalInput>
