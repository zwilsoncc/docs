...
If that fits your project, simply add the <Link href="/docs/features/configuration#`env`-(object)">env property</Link> to your `now.json` file or to the `vercel` property inside `package.json`:
...
Sometimes you need to store sensitive information on the deployment that should only be accessible by the code that's running in it. This can be accomplished using `vercel secret`, which allows you to store such data needed by your apps to function (such as API tokens or passwords) in a secure way.
...
Once it's created, you can rename it with `vercel secret rename` or delete it completely with `vercel secret rm`. For more examples and the full list of options and commands, run `vercel help secret`.
