...
Aliases which will be assigned to the latest deployment when running `vc alias` (with no arguments).
...
A field for specifying the deployment type ("npm", "docker" or "static") if both `package.json` and `Dockerfile` exist. This will prevent `vc` from asking you to choose the type in these cases.
