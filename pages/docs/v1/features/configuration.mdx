...
export const meta = {
  title: `Configuring ${PRODUCT_SHORT_NAME_V1}`,
  description: `Configuring ${PRODUCT_SHORT_NAME_V1} so that your deployments are exactly as you need them`,
  date: '22 Feb 2017',
...
Specifies the [<ProductShortNameV1 /> Platform version](/docs/v2/platform/overview#versioning) the deployment should use and is known to work with. We strongly recommend setting a `version` when working on production systems or using source control (e.g. [Git](https://git-scm.com)).

The default version for <ProductShortNameV1 /> deployments is [version 2](/docs/v1-upgrade).

To explicitly use [version 1 of <ProductShortNameV1 />](/blog/now-2), use the following configuration in a `now.json` file.
...
You can also instruct <ProductShortNameV1 /> to automatically
...
For an example, this is how to use the latest version of Node.js 6 inside <ProductShortNameV1 />.
...
The URL of the <ProductShortNameV1 /> API which <CliNameV1 /> should interact with.
...
for all of your providers accessed through a <ProductShortNameV1 /> client.
