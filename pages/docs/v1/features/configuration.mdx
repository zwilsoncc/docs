...
export const meta = {
  title: `Configuring ${PRODUCT_SHORT_V1}`,
  description: `Configuring ${PRODUCT_SHORT_V1} so that your deployments are exactly as you need them`,
  date: '22 Feb 2017',
  editUrl: 'pages/docs/v1/features/configuration.mdx',
  lastEdited: '2020-04-13T15:52:00.000Z'
}

This guide explains how to use these methods to configure and customize the default behavior of <CliNameV1 />.
...
This describes the set of configuration rules that are applied per project. The configuration file always lives in the root directory of the project (except when a custom path is specified using the `--local-config` flag in <CliNameV1 />).
...
Specifies the [<ProductNameShortV1 /> Platform version](/docs/v2/platform/overview#versioning) the deployment should use and is known to work with. We strongly recommend setting a `version` when working on production systems or using source control (e.g. [Git](https://git-scm.com)).

The default version for <ProductNameShortV1 /> deployments is [version 2](/docs/v1-upgrade).

To explicitly use [version 1 of <ProductNameShortV1 />](/blog/now-2), use the following configuration in a `now.json` file.
...
You can also instruct <ProductNameShortV1 /> to automatically
...
For an example, this is how to use the latest version of Node.js 6 inside <ProductNameShortV1 />.
...
The URL of the <ProductNameShortV1 /> API which <CliNameV1 /> should interact with.
...
This describes the set of configuration rules that apply to <CliNameV1 />.
...
the `--global-config` flag in <CliNameV1 />, which takes in a
...
they're being used for. As an example, this will make <CliNameV1 /> receive
canary updates:
...
for all of your providers accessed through a <ProductNameShortV1 /> client.
