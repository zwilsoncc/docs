...
export const meta = {
  ...
  lastEdited: '2020-04-13T08:11:43.000Z'
}

This guide explains how to use these methods to configure and customize the default behavior of <CliV1 />.
...
This describes the set of configuration rules that are applied per project. The configuration file always lives in the root directory of the project (except when a custom path is specified using the `--local-config` flag in <CliV1 />).
...
The URL of the <ProductNameShortV1 /> API which <CliV1 /> should interact with.
...
This describes the set of configuration rules that apply to <CliV1 />.
...
the `--global-config` flag in <CliV1 />, which takes in a
...
they're being used for. As an example, this will make <CliV1 /> receive
canary updates:
