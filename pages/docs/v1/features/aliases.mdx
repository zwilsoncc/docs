...
Aliases are managed by invoking the command `vc alias`, which has the following syntax:
...
It's usually known as the **deployment ID**, which is also accessible when you list your deployments with `vc ls`. It can be used in substitution of the URL when running `vc alias`:

<TerminalInput>
  now alias <b>xpvB6GZWMR6Q9Oxe3c64Htgq</b> get-started-basic
</TerminalInput>

For convenience, we parse the URLs you supply as parameters to `vc alias` to extract the hostname, which means the following are all valid **source URLs**:
...
After executing the `alias` above, just run `vc domains ls` to see all the domains in your account:
...
- **zeit.world**: the domain is hosted with <Link href="/world">zeit.world</Link> and `vc alias` automatically sets up records.

Once a domain appears under `vc domains ls`, **only you can point aliases to it** until you remove it from your account.
