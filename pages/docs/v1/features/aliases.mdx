...
export const meta = {
  ...
  description: `All you need to know about assigning domains to your ${PRODUCT_SHORT_V1} deployments`,
  date: '21 Feb 2017',
  editUrl: 'pages/docs/v1/features/aliases.mdx',
  lastEdited: '2020-04-13T06:15:21.000Z'
}

This guide covers how to assign two types of domains to your <ProductNameShortV1 /> deployments.
...
Whenever you deploy to <ProductNameShortV1 />, the URL you get in return looks something like:
...
​<ProductNameShortV1 /> aliases have two purposes:
...
If `your-company.com` is available to buy, <ProductNameShortV1 /> will allow you to buy it directly from the command line, which is the only step you need to do. <ProductNameShortV1 /> will take care of all the domain configuration.

Otherwise, you need to [configure](/docs/getting-started/assign-a-domain-name) the domain name for <ProductNameShortV1 />. You can either use our [global DNS service](/world) or add an `ALIAS/CNAME` record on your existing DNS solution.
...
- **zeit.world**: the domain is hosted with <Link href="/world">zeit.world</Link> and `now alias` automatically sets up records.
