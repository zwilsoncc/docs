...
export const meta = {
  ...
  description: `All you need to know about assigning domains to your ${PRODUCT_SHORT_V1_NAME} deployments`,
  date: '21 Feb 2017',
  editUrl: 'pages/docs/v1/features/aliases.mdx',
  lastEdited: '2020-05-08T23:58:00.000Z'
}

This guide covers how to assign two types of domains to your <ProductShortV1Name /> deployments.
...
Whenever you deploy to <ProductShortV1Name />, the URL you get in return looks something like:

<TerminalOutput>
  https://<b>get-started-basic</b>-<b>etjnigdrkz</b>.now.sh
</TerminalOutput>
...
​<ProductShortV1Name /> aliases have two purposes:
...
Aliases are managed by invoking the command `vercel alias`, which has the following syntax:
...
<TerminalInput>
  now alias https://get-started-basic-etjnigdrkz.now.sh <b>get-started-basic</b>
</TerminalInput>
...
<Image
  src={`${process.env.IMAGE_ASSETS_URL}/docs/aliases/alias-dest.png`}
  width="682"
  height="468"
  caption={
    <span>
...
It's usually known as the **deployment ID**, which is also accessible when you list your deployments with `vercel ls`. It can be used in substitution of the URL when running `vercel alias`:

<TerminalInput>
  now alias <b>xpvB6GZWMR6Q9Oxe3c64Htgq</b> get-started-basic
</TerminalInput>

For convenience, we parse the URLs you supply as parameters to `vercel alias` to extract the hostname, which means the following are all valid **source URLs**:
...
If `your-company.com` is available to buy, <ProductShortV1Name /> will allow you to buy it directly from the command line, which is the only step you need to do. <ProductShortV1Name /> will take care of all the domain configuration.

Otherwise, you need to [configure](/docs/getting-started/assign-a-domain-name) the domain name for <ProductShortV1Name />. You can either use our [global DNS service](/world) or add an `ALIAS/CNAME` record on your existing DNS solution.
...
After executing the `alias` above, just run `vercel domains ls` to see all the domains in your account:
...
- **zeit.world**: the domain is hosted with <Link href="/world">zeit.world</Link> and `vercel alias` automatically sets up records.

Once a domain appears under `vercel domains ls`, **only you can point aliases to it** until you remove it from your account.
