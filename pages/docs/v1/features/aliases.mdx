...
This guide covers how to assign two types of domains to your <ProductShortNameV1 /> deployments.
...
Whenever you deploy to <ProductShortNameV1 />, the URL you get in return looks something like:
...
​<ProductShortNameV1 /> aliases have two purposes:
...
If `your-company.com` is available to buy, <ProductShortNameV1 /> will allow you to buy it directly from the command line, which is the only step you need to do. <ProductShortNameV1 /> will take care of all the domain configuration.

Otherwise, you need to [configure](/docs/getting-started/assign-a-domain-name) the domain name for <ProductShortNameV1 />. You can either use our [global DNS service](/world) or add an `ALIAS/CNAME` record on your existing DNS solution.
