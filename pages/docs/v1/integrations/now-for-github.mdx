...
For example, the following `now.json` configuration will make <ProductShortV1Name /> for GitHub alias the most recent push to `my-zeit-website.com` by default.
...
### Disabling <ProductNameWrapper name={PRODUCT_SHORT_V1_NAME} /> for GitHub with `now.json`
