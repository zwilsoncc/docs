...
import { CLI_NAME_V1 } from '~/lib/constants'
import CliNameV1 from '~/components/product-name/cli-name-v1'

export const meta = {
  title: CLI_NAME_V1,
  description: `Using ${CLI_NAME_V1} to create new deployments or control existing ones`,
  date: '09 Mar 2017',
  editUrl: 'pages/docs/v1/clients/now-cli.mdx',
  lastEdited: '2020-04-13T20:57:18.000Z'
}

The `now` CLI is the main interface to the <ProductShortNameV1 /> platform.
...
The following are the most important commands in <CliNameV1 />:

| Syntax                                                  | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| ------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| <InlineCode noWrap>now [path] ...</InlineCode>          | When you invoke `now`, the files within the current directory will be uploaded to <ProductShortNameV1 /> and a new deployment will be created.<br/><br/>After that, you'll instantly receive its URL so that you can share it with other people around the globe. <br/><br/> The `[path]` can either be a file (leads to a static deployment) or a directory (the deployment type will be determined depending on the contents). If you provide `[path]`, you'll need to use the parameter `--local-config=../now.json` or just `-A ../now.json` to tell the CLI where to read the configuration. <br/><br/> If you want to deploy multiple paths at once, [read this](#deploying-multiple-projects-at-once). |
...
| <InlineCode noWrap>now certs [cmd]</InlineCode>         | By default, <ProductShortNameV1 /> will automatically provision certificates for your deployments. Using this subcommand, you can see when they're expiring and upload your own ones ([read more](/docs/v1/features/certs)).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
​<ProductShortNameV1 /> uses the metafiles inside your project to determine which files and directories should be uploaded to our platform and which ones should be ignored:
