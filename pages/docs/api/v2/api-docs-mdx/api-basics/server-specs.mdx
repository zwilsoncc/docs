export const meta = {
  editUrl: 'pages/docs/api/v2/api-docs-mdx/api-basics/server-specs.mdx',
  lastEdited: '2019-03-19T12:50:31.000Z'
}

## Server Specs

### Origins

The Now API serves endpoints from Origin nodes located both in [bru1, hnd1, iad1, sfo1](/docs/v2/platform/regions-and-providers).

Each request to any `api.zeit.co` endpoint, will be automatically resolved to the closest Origin location using [Anycast](https://zeit.co/network). Allowing requests to be routed this way is the recommended way to use our API.

If requests are desired to hit a certain location, however, the API can be accessed through individual URLs for each Origin. SFO1 can be resolved through `api-sfo1.zeit.co`, and BRU1 can be resolved through `api-bru1.zeit.co`.

### HTTP and TLS

The API supports HTTP versions 1, 1.1, and 2, although HTTP/2 is preferred.

TLS version 1.2 is supported.

For more information on TLS support, refer to the [SSL Labs report](https://www.ssllabs.com/ssltest/analyze.html?d=api.zeit.co&hideResults=on&latest).
