...
| **builds**    | <HelpLink href="#api-basics/types">List</HelpLink>         | No       | A list of Build objects used to build sources in a deployment. For example, <InlineCode noWrap>{`[{ "src": "*.js", "use": "@now/node" }]`}</InlineCode>.                                                                        |
...
<Note>
  ...
  get created during the initialization of the deployment. This could make the deployment
...
<span>Example successful (<b>200</b>) response:</span>
<Code lang="json">{`{
...
<Example>
  <span>
    Example successful (<b>200</b>) response:
  </span>
  <Code lang="json">{`{
  "id": "dpl_6EZWg2tkQevMN76UQzFgKsD2gaNR",
  "url": "my-instant-deployment-2i7mwl1a1w.now.sh",
...
<b>Example Second Deployment using <InlineCode>?forceNew=1</InlineCode></b>
<P>Request:</P>
...
<Endpoint>
  DELETE /v9/now/deployments/:id
  <br />
  DELETE /v9/now/deployments/remove?url
</Endpoint>
...
<Example>
  ...
  />
  <span>
    Example Successful (<P>200</P>) response:
  </span>
  <Code lang="json">{`{
  "builds": [
    {
