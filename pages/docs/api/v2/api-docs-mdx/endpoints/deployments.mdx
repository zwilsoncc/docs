...
export const meta = {
  ...
  lastEdited: '2019-09-30T13:07:32.000Z'
}
...
<Request
  method="DELETE"
  url="https://api.zeit.co/v9/now/deployments/dpl_5WJWYSyB7BpgTj3EuwF37WMRBXBtPQ2iTMJHJBJyRfd"
  auth
/>
...
<Endpoint method="GET" url="/v9/now/deployments/:id/files" />
...
<Request
  url="https://api.zeit.co/v9/now/deployments/dpl_BRGyoU2Jzzwx7myBnqv3xjRDD2GnHTwUWyFybnrUvjDD/files"
  auth
/>
...
<Endpoint method="GET" url="/v9/now/deployments/:id/files/:fileId" />
...
<Endpoint method="GET" url="/v9/now/deployments/:id/builds" />
...
| **fingerprint**  | <HelpLink href="#api-basics/types">String</HelpLink>       | If the Build uses the `@now/static` Builder, it contains a hashed string of all outputs.                                                                                                                  |
...
| **lambda** | <HelpLink href="#api-basics/types">Map</HelpLink>          | If the output is a Serverless Function, it returns an object that contains `functionName` and `deployedTo` keys. |
