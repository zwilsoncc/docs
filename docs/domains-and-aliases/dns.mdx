...
<Note>This only applies to domains you bought via <Now color="#000" /> or domains configured with our <GenericLink href="/dns">global DNS service</GenericLink>.</Note>
...
> 1. use a custom certificate for your <Now color="#000" /> deployment with <GenericLink href="/docs/features/certs#now-certs-replace">`now certs replace`</GenericLink>
...
The endpoint is called `/domains/:domain/records` and is available in our <GenericLink href="/api#get-domain-records">REST API</GenericLink>.

<Card title="Read Next: Managing SSL Certificates" href="/docs/v2/domains-and-aliases/ssl-certificates">
  Learn how to manage the SSL certificates for your domain.
</Card>
