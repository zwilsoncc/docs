...
export const meta = {
  title: 'Official Static Builds Builder (@now/static-build)',
...
This module takes a Node.js `package.json` file, installs all its dependencies, executes the `now-build` step and exposes the resulting `dist` directory as the [build output](/docs/v2/deployments/builds#source-and-outpu) for serving.
