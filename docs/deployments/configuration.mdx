...
The parameters stored in that file are passed to the [Deployment Creation API endpoint](/api/v2#endpoints/deployments/create-a-new-deployment), which performs validation and stores the metadata in our backend.
...
**Valid values**: [domain names](/docs/v2/domains-and-aliases/domains) (optionally including subdomains) added to the account.
...
<Example>
<P.B>Usage Example:</P.B>
<PDIV>This example will pass the `MY_KEY` static env to all <GenericLink href="/docs/v2/deployments/concepts/lambdas">Lambdas</GenericLink> and `SECRET` resolved from the `my-secret-name` <GenericLink href="/docs/v2/deployments/environment-and-secrets">secret</GenericLink> dynamically.</PDIV>
<Code>{`{
  "env": {
...
<Code>{`{
  ...
  "routes": [
    { "src": "/custom-page", "dest": "index.html" },
