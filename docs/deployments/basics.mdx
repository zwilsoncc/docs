...
The deployment process gets triggered by either an [API call](/api), the `now` command, or `git push` if using our [GitHub integration](/docs/v2/integrations/now-for-github).
...
It is important to understand that Now keeps track of [**sources and outputs**](/docs/v2/deployments/builds#source-and-output). If we add the [special pathname](/docs/v2/deployments/concepts/special-paths) `/_src` at the end of the deployment, we will be able to inspect them:

Notice that a `date.js` lambda was built (identified with the icon λ), but `index.html` is no longer present. This is a precaution Now takes when using Builds to prevent source files from being accidentally exposed to the outside world.
...
<Card title="Read More: Extending Now Deployments with Builds" href="/docs/v2/deployments/builds">
