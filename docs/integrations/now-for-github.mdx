...
Disabling Now for GitHub with this configuration option will prevent Now from [deploying any push](#a-deployment-for-each-push) or [aliasing the changes](#aliasing-the-main-branch) when merged to the default branch.

#### Disabling Auto-Aliasing
To stop Now from [automatically aliasing the default branch](#aliasing-the-main-branch) to any alias setup within the `now.json` file, use the following configuration option.
```
{
  "alias": ["my-zeit-website.com"],
  "github": {
...
<Note>Although Now will stop commenting, it will not stop deploying the latest updates. For this configuration option, see <GenericLink href="#configuration/disabling-now-for-github-with">disabling Now for GitHub</GenericLink></Note>
