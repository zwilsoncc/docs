...
export const meta = {
  description: 'Using Now with a scope of a user or a team.'
}
...
When communicating with [our API](/api), all interactions only consider the resources contained within your own user account by default.
...
However, you still need to attach a token of your user account as described [here](/api#api-basics/authentication). In turn, this user account needs to be a member of the team you would like to access over the API.

<Note>For more information on the teams API endpoint, see our <GenericLink href="/api/v2#endpoints/teams">API reference documentation</GenericLink>.</Note>
