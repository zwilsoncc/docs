...
- Cost of logging (no more storage costs, charged [per-line](what-is-a-line-ref) and [stricly](logs-limit) [ephemeral](logs-ephemeral)).
...
From now on, the deployment `type` is no longer used and not exposed via the relevant [GET API endpoint](/api/v2#endpoints/deployments/get-a-single-deployment).
...
- Faster cold boot-up times due to refined [entrypoint granularity](/docs/v2/deployments/concepts/per-entrypoint-granularity)
...
In other words, if you are writing `new Server()` (in whatever language you prefer), you might be missing out on the [**granularity benefits**](/docs/v2/deployments/concepts/per-entrypoint-granularity) of serverless.
