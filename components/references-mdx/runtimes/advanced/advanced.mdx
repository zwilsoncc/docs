...
<Note type="warning">
  Using Rails is not recommended because it is not optimized for a serverless environment. You are likely to find
  size or cold start limitations.
</Note>
...
<Caption>
  An example <InlineCode>now.json</InlineCode> that uses the Ruby Runtime for all files with a <InlineCode>.ru</InlineCode> suffix.
</Caption>
