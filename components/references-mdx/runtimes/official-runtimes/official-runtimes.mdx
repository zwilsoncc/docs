...
When creating a new deployment, it automatically receives the latest LTS Node.js version available on ZEIT Now at that point in time (run `node -v` to understand which one).

In order to customize it, add a `package.json` file with an `engines` field.

The available Node.js versions are either **10**, or **12**, and can be configured like so:
...
The Python Runtime is used by Now to compile Python Serverless Functions, that defines a singular HTTP `handler` variable, inheritting from the `BaseHTTPRequestHandler` class, from a `.py` file within an `/api` directory at your project's root.
...
The Ruby Runtime is used by Now to compile Ruby Serverless Functions that define a singular HTTP handler from `.rb` files within an `/api` directory at your project's root.
