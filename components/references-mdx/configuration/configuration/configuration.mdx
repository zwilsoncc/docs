...
<Note type="alert">
  ...
  you run <InlineCode>vc</InlineCode>.
</Note>
...
Furthermore, it also affects any other actions that the user takes within the directory that contains this configuration (e.g. listing [Environment Variables](/docs/v2/build-step#environment-variables) using `vc secrets ls`).
