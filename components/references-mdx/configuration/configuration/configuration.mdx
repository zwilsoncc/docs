...
The alias or aliases are applied automatically using [ZEIT Now for GitHub](/docs/v2/more/now-for-github), [ZEIT Now for GitLab](/docs/v2/more/now-for-gitlab), or [ZEIT Now for Bitbucket](/docs/v2/more/now-for-bitbucket) when merging or pushing to the default branch.
...
When set to `true`, all HTML files will have their `.html` extension removed. When visiting a path that ends with `.html`, a 308 response will redirect the client to the extensionless path.

For example, a file named `about.html` will be served when visiting the `/about` path. Visiting `/about.html` will redirect to `/about`.
...
When set to `true`, visiting a path that does not end with a forward slash will respond with a 308 status code and redirect to the path with a trailing slash.

When set to `false`, visiting a path that ends with a forward slash will respond with a 308 status code and redirect to the path without the trailing slash.

For example, when `trailingSlash: false` the `/about/` path will redirect to `/about`.
...
<Code lang="json">{`{
  "redirects": [
    ...
    { "source": "/user", "destination": "/api/user", "statusCode": 308 },
