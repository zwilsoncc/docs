...
<Note type="alert">
  ...
  <Link href="/docs/cli#commands/overview/project-linking">
    Project Linking
  </Link>
  , which allows you to link a <ProductName /> Project to your local codebase when
  you run <InlineCode>vercel</InlineCode>.
</Note>
...
**Valid values**: [domain names](/docs/v2/custom-domains) (optionally including subdomains) added to the account, or a string for a suffixed URL using `.vercel.app` or a Custom Deployment Suffix ([available on the Enterprise plan](/pricing)).
...
<Note>
  Aliases from a <InlineCode>vercel.json</InlineCode> file will be{' '}
  <b>ignored</b> when there are domains that have been{' '}
...
Furthermore, it also affects any other actions that the user takes within the directory that contains this configuration (e.g. listing [Environment Variables](/docs/v2/build-step#environment-variables) using `vercel secrets ls`).
...
<Note>
  There's no need to use this property for deploying your frontend. If you'd
  like to customize your Serverless Functions, use the{' '}
  <Link href="/docs/configuration#project/functions">functions</Link> property
  instead.
  <br />
  <br />
  The only case in which the use of <InlineCode>builds</InlineCode> is still recommended
  is if your Git repository contains multiple different frontends that you would
  like to mount to different paths in your URL. True monorepos (mounting multiple
  frontends to different domains) are currently not supported.
</Note>
...
<Note>
  ...
  a security precaution. This is why we need to allowlist static files
  explicitly with
  <InlineCode>@vercel/static</InlineCode>.
</Note>
...
In the example above, the `api/test.php` Serverless Function does not use one of the [officially supported runtimes](/docs/v2/serverless-functions/supported-languages/). In turn, a `runtime` property was added in order to invoke the [vercel-php](https://www.npmjs.com/package/vercel-php) community runtime.
...
<Note type="warning">
  It is not possible to use <InlineCode>routes</InlineCode> in combination with
  the following properties:{' '}
  <Link href="/docs/configuration#project/cleanurls">cleanUrls</Link>,{' '}
  ...
  <Link href="/docs/configuration#project/rewrites">rewrites</Link>, and{' '}
...
<Note type="warning">
  Deploying Serverless Functions to multiple regions is limited to [Enterprise plans](/pricing).
  Read more about this limit on <Link href="/docs/v2/platform/limits#serverless-function-regions">the Limits page</Link>.
</Note>
...
- `permanent`: A boolean to toggle between permanent and temporary redirect (default true). When `true`, the status code is [308](https://developer.mozilla.org/en-US/docs/Web/HTTP/Status/308). When `false` the status code is [307](https://developer.mozilla.org/en-US/docs/Web/HTTP/Status/307).
