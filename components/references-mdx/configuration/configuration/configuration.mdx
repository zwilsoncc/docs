...
For example, when `trailingSlash: true`, the `/about` path will redirect to `/about/`.

However, paths that contain a `.` will not redirect.

For example, visting `/about/style.css` will not redirect but `/about/style/` will redirect to `/about/style/`
...
When `undefined`, visiting a path with or without a trailing slash will not redirect.
