...
<Note>
  ...
  for a better experience. However, their feature set is still being
  improved, so you may use{' '}
  <Link href="/docs/configuration#project/routes">routes</Link> if they suit your
  needs better.
</Note>
...
<Code lang="json">{`{
  "rewrites": [
    ...
    { "source": "/user/:id", "destination": "/api/user?uid=:id" },
