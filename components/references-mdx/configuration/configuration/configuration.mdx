...
When set to `true`, all HTML files will have their `.html` extension removed. When visiting a page that ends with `.html`, a 308 response will redirect the client to the extensionless page.

For example, a file named `about.html` will be served when visiting the `/about` page. Visiting `/about.html` will redirect to `/about`.
