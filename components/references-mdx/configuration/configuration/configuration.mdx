...
[Environment variables](/docs/v2/build-step/#using-environment-variables-and-secrets) passed to the [Build](/docs/v2/advanced/runtimes) processes.

<P>
  The following example will pass the <InlineCode>MY_KEY</InlineCode> static
  env to all{' '}
  <Link href="/docs/v2/advanced/runtimes">Builds</Link> and
...
- `use` (`String`): A npm module to be installed by the [build process](/docs/v2/advanced/runtimes). It can include a semver compatible version (e.g.: `@org/proj@1`).
- `config` (`Object`): Optionally, an object including arbitrary metadata to be passed to the Runtime.

<P>
  The following will include all HTML files as-is (to be served statically),
  and <Link href="/docs/v2/advanced/runtimes">Build</Link> all
...
For more information on Builds and Runtimes, see the documentation:

<Card title="Runtimes" href="/docs/v2/advanced/runtimes">
