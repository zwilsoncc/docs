...
<Note>
  There's no need to use the <InlineCode>builds</InlineCode> property for
  deploying your frontend. If you'd like to customize your Serverless Functions,
  use the <Link href="/docs/configuration#project/functions">functions</Link>{' '}
  property instead.
  <br />
  <br />
  The only case in which the use of <InlineCode>builds</InlineCode> is still recommended
  is if your Git repository contains multiple frontends that you would like to mount
  to different paths in your URL. Mounting multiple frontends to different domains
