...
<Note>
  ...
  <Link href="/docs/runtimes#official-builders/node-js">@vercel/node</Link>{' '}
  Runtime as an example of a <InlineCode>vercel.json</InlineCode> configuration
  that tells <ProductName /> to build JavaScript files with Node.js and outputs
  them as{' '}
...
<Note>
  If you are not using a framework, you can emit a{' '}
  <InlineCode>404.html</InlineCode> file to the{' '}
  <Link href="/docs/v2/build-step#output-directory">Output Directory</Link>.
</Note>
...
<Caption>
  ...
  <InlineCode>other-404.html</InlineCode> as the 404 page for any path not
  matched in the filesystem.
</Caption>
