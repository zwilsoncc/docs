...
<Note>
  ...
  <Link href="#advanced-usage/developing-your-own-builder">
...
Using a file with a `.sh` file extension as an entry point for the `@now/static-build` Builder allows you to define a custom build script.
...
<Caption>
  Using the{' '}
  <Link href="/docs/v2/deployments/official-builders/node-js-now-node/">
    @now/node Builder
  </Link>{' '}
