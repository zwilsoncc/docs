...
This section contains information about the two types of options available to pass when using [<ProductNameShort /> CLI](/download) commands, [unique](#unique-options) and [global](#global-options).
...
Unique options relate only to a single <ProductNameShort /> CLI command and as such, are documented in full in the command section they relate to.
...
Global options are commonly available to use with multiple <ProductNameShort /> CLI commands.

### Debug

The `--debug` option, shorthand `-d`, can be used to provide a more verbose output when running <ProductNameShort /> CLI commands.
...
The `--help` option, shorthand `-h`, can be used to display more information about [<ProductNameShort /> CLI](/downlaod) commands.
...
The `--scope` option, shorthand `-S`, can be used to execute [<ProductNameShort /> CLI](/download) commands from a different team or user account than is currently active.
...
The `--token` option, shorthand `-t`, can be used to execute [<ProductNameShort /> CLI](/download) commands with an [authorization token](/account/tokens).
