...
## <ProductNameWrapper name={PRODUCT_SHORT} />
...
When running `now` in a directory for the first time, [<ProductNameShort /> CLI](/download) needs to know which [scope](/docs/now-cli#options/global-options/scope) and [Project](/docs/v2/platform/projects) you
...
<Note>
  ...
  . If you have an earlier version, please{' '}
...
When you create a new project, <ProductNameShort /> CLI will automatically detect the framework you are using and offer
...
The `--no-clipboard` option, shorthand `-C`, can be used to prevent <ProductNameShort /> CLI from copying the deployment URL to the clipboard.
...
The `--version` option can be used to verify the version of [<ProductNameShort /> CLI](/download) currently being used.
