...
<OutputTable>
  ...
  <Row>
    ...
    <TypeCell>Date</TypeCell>
    <Cell>
      If it was purchased through {<ProductShortV1Name />} the date when it was
      purchased.
    </Cell>
  </Row>
  <Row>
...
Register a new domain name with <ProductShortV1Name /> for the authenticating
...
Delete a previously registered domain name from <ProductShortV1Name />.
...
<Request
  method="DELETE"
  url={`${API_ENDPOINT}/v2/domains/example.com`}
  headers={{
    Authorization: `Bearer ${
      props.testingToken ? props.testingToken.token : '$TOKEN'
    }`
...
<Request
  url={`${API_ENDPOINT}/v2/domains/status?name=example.com`}
  headers={{
    Authorization: `Bearer ${
      props.testingToken ? props.testingToken.token : '$TOKEN'
    }`
...
<Request
  url={`${API_ENDPOINT}/v2/domains/price?name=example.com`}
  headers={{
    Authorization: `Bearer ${
      props.testingToken ? props.testingToken.token : '$TOKEN'
    }`
...
<Request
  method="POST"
  url={`${API_ENDPOINT}/v2/domains/buy`}
  headers={{
    Authorization: `Bearer ${
      props.testingtoken ? props.testingtoken.token : '$token'
    }`,
