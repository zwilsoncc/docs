...
export const meta = {
  editUrl: 'pages/docs/api/v1/api-docs-mdx/endpoints/deployments.mdx',
  lastEdited: '2019-05-25T14:12:19.000Z'
}
...
Before you create the deployment, you need to [upload any required files](#endpoints/deployments/upload-deployment-files) when they cannot be posted at once.

<Note>The code and logs under the OSS plan will be public.</Note>
...
<InputTable>
  <Row>
    ...
    <Cell>
      An object containing the deployment's environment variable names and
      values.{' '}
      <i>
        Secret may be referenced by prefixing the value with{' '}
  ...
  <Row>
    ...
    <Cell>
      A boolean indicating if the deployment is public. For every deployment
      done under the OSS plan, this needs to be set to{' '}
  ...
  <Row>
    ...
    <BooleanCell status={false} />
    <Cell>
      A boolean indicating if a new deployment should be forced (this create a
      new deployment even if the files are the same as the latest one).
    </Cell>
  </Row>
  <Row>
  ...
  <Row>
    ...
    <BooleanCell status={true} />
    <Cell>
      A string indicating the type of deployment, it could be{' '}
      <InlineCode>NPM</InlineCode>, <InlineCode>DOCKER</InlineCode> or{' '}
  ...
  <Row>
    ...
    <BooleanCell status={false} />
    <Cell>
      A map with the desired Node.js version (<i>only for NPM deployments</i>)
      defined as the{' '}
      <Link href="https://docs.npmjs.com/files/package.json#engines">
  ...
  <Row>
    ...
    <BooleanCell status={false} />
    <Cell>
      A string indicating the session affinity algorithm. It could be{' '}
      <InlineCode>ip</InlineCode> or <InlineCode>random</InlineCode>.
    </Cell>
  </Row>
  <Row>
    ...
    <BooleanCell status={false} />
    <Cell>
      An object containing any{' '}
      <Link href="/docs/features/configuration#local-configuration">
        local configuration
      </Link>{' '}
      property. Not all of these properties lead to changes in behavior on the
      platform (some are designed to be used only by <CliV1Name />
      ).
    </Cell>
  </Row>
</InputTable>
...
<InputTable>
  <Row>
    ...
    <BooleanCell status={true} />
    <Cell>
      The file name including the whole path, eg.{' '}
      <InlineCode>folder/file.js</InlineCode>.
    </Cell>
  </Row>
  <Row>
    ...
    <BooleanCell status={true} />
    <Cell>
      The file content, it could be either a base64 (useful for images, etc.) of
      the files or the plain content for source code.
    </Cell>
  </Row>
  <Row>
    ...
    <BooleanCell status={false} />
    <Cell>
      The file encoding, it must be <InlineCode>base64</InlineCode> or{' '}
      <InlineCode>utf-8</InlineCode>. Defaults to <InlineCode>utf-8</InlineCode>.
    </Cell>
  </Row>
</InputTable>
...
> **Note**: [Upload each file](#endpoints/deployments/upload-deployment-files) and then create the deployment with the SHA1 string is specially useful when you have to either upload many or big files. That allow you to retry file uploads in case of a network error.
...
<Request
  url={`${API_ENDPOINT}/v3/now/deployments`}
  method="GET"
  headers={{
...
<HeadersTable>
  <Row>
    ...
    <Cell>
      With the value <InlineCode>application/octet-stream</InlineCode>.
    </Cell>
  </Row>
  <Row>
...
If a deployment has not finished uploading (is incomplete), the `url` property will have a value of `null`.
...
<OutputTable>
  <Row>
    ...
    <Cell>
      A string with the unique deployment ID you can use to get more information
      or remove it.
    </Cell>
  </Row>
  <Row>
  ...
  <Row>
    ...
    <TypeCell>String</TypeCell>
    <Cell>
      A string with the unique URL of the deployment. If it hasn't finished
      uploading (is incomplete), the value will be <InlineCode>null</InlineCode>
      .
    </Cell>
  </Row>
  <Row>
    ...
    <TypeCell>String</TypeCell>
    <Cell>
      A number containing the date when the deployment was created (
      <i>in timestamp</i>).
    </Cell>
  </Row>
  <Row>
  ...
  <Row>
    ...
    <TypeCell>Enum</TypeCell>
    <Cell>
      A string with the current deployment state, it could be one of the
      following <InlineCode>DEPLOYING</InlineCode>,{' '}
      <InlineCode>DEPLOYMENT_ERROR</InlineCode>, <InlineCode>BOOTED</InlineCode>
      , <InlineCode>BUILDING</InlineCode>, <InlineCode>READY</InlineCode>,{' '}
      <InlineCode>BUILD_ERROR</InlineCode>, or <InlineCode>FROZEN</InlineCode>.
    </Cell>
  </Row>
  <Row>
    ...
    <TypeCell>Enum</TypeCell>
    <Cell>
      A string defining the deployment type, it could be:{' '}
      <InlineCode>NPM</InlineCode>, <InlineCode>DOCKER</InlineCode> or{' '}
      <InlineCode>STATIC</InlineCode>.
    </Cell>
  </Row>
  <Row>
    <BoldCell>meta</BoldCell>
    <TypeCell>Map</TypeCell>
    <Cell>A map containing all deployment metadata.</Cell>
  </Row>
  <Row>
    ...
    <Cell>
      ...
      <Link href="/docs/getting-started/scaling">scale configuration</Link> of
      the deployment.
    </Cell>
  </Row>
</OutputTable>
...
<Request
  url={`${API_ENDPOINT}/v2/now/deployments`}
  headers={{
    Authorization: `Bearer ${
      props.testingToken ? props.testingToken.token : '$TOKEN'
    }`
...
You can obtain this, for example, by [listing all deployments](#endpoints/deployments/get-/now/deployments).
...
<Request
  url={`${API_ENDPOINT}/v2/now/deployments/ChbZiZe84CKtod6rmCIRRvYR`}
  headers={{
    Authorization: `Bearer ${
      props.testingToken ? props.testingToken.token : '$TOKEN'
    }`
...
<OutputTable>
  ...
  <Row>
    ...
    <TypeCell>Enum</TypeCell>
    <Cell>
      If it's a <InlineCode>file</InlineCode> or a{' '}
  ...
  <Row>
    ...
    <TypeCell>ID</TypeCell>
    <Cell>
      The unique ID of the file (only valid for <InlineCode>file</InlineCode>{' '}
      type).
    </Cell>
  </Row>
  <Row>
    ...
    <TypeCell>List</TypeCell>
    <Cell>
      The children files of the directory (only valid for{' '}
      <InlineCode>directory</InlineCode> type).
    </Cell>
  </Row>
</OutputTable>
...
<Request
  url={`${API_ENDPOINT}/v2/now/deployments/ChbZiZe84CKtod6rmCIRRvYR/files`}
  headers={{
    Authorization: `Bearer ${
      props.testingToken ? props.testingToken.token : '$TOKEN'
    }`
...
<Request
  method={`${API_ENDPOINT}/v2/now/deployments/ChbZiZe84CKtod6rmCIRRvYR/files/2d4aad419917f15b1146e9e03ddc9bb31747e4d0`}
  headers={{
    Authorization: `Bearer ${
      props.testingToken ? props.testingToken.token : '$TOKEN'
    }`
...
<Code lang="js">{`require("http").Server((req, res) => {
    res.setHeader("Content-Type", "text/plain; charset=utf-8");
    res.end(require("sign-bunny")("Hi there!"));
  }).listen();`}</Code>
</Example>
...
<OutputTable>
  ...
  <Row>
    ...
    <TypeCell>String</TypeCell>
    <Cell>
      The instance unique url. Is the deployment unique URL with the instance{' '}
      <InlineCode>uid</InlineCode> added before <InlineCode>.now.sh</InlineCode>
      .
    </Cell>
  </Row>
</OutputTable>
...
<Request
  url={`${API_ENDPOINT}/v1/now/deployments/DguxFCKb5RdNmTw20KGwb42H/instances`}
  headers={{
    Authorization: `Bearer ${
      props.testingToken ? props.testingToken.token : '$TOKEN'
    }`
...
> **Note**: The deployment _must_ be on the `READY` state to scale it.
...
<Request
  url={`${API_ENDPOINT}/v1/now/deployments/2sS6QiQppD9moBzhmX34Y21H/instances`}
  method="POST"
  headers={{
    Authorization: `Bearer ${
      props.testingToken ? props.testingToken.token : '$TOKEN'
