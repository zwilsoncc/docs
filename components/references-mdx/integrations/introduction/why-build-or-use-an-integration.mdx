...
An example of this is the [Google Cloud Integration](/integrations/gcloud) which connects projects on ZEIT to Google Cloud Platform projects where you can run ZEIT Now deployments as CRON jobs through the Google Cloud Scheduler, Create and connect to MySQL/PostgreSQL databases with Google Cloud SQL, and much more that would be a lot harder without an Integration.
