...
Every request to a UIHook will receive a token which can be used to make requests to the <ProductNameSimple /> API for the related account.
...
When you use the `@zeit/integration-utils` utility, you **do not** need to extract this token manually. You will receive an instance called `zeitClient` which is initialized with the above token and account information. You can use that to consume the <ProductNameSimple /> token.
