...
Sometimes, your Integration will use a [UIHook](/docs/integrations#understanding-uihooks) but also need to access the <OrgName /> API in a background worker.
...
- `next` - The installation URL of your Integration in the <OrgName /> Dashboard
...
Now you can use UIHook's to display reports and receive inputs from the user. You can also use the obtained `accessToken` to access the <OrgName /> API from anywhere, at anytime.
