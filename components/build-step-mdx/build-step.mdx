...
Adding **Now Secrets** can be done with [<ProductNameShort /> CLI](/download), providing three options to work with them.

<Note>
  When adding Now Secrets with <ProductNameShort /> CLI, the secret name is
  automatically lowercased before being stored.
</Note>
