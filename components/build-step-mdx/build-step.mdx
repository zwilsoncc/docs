...
<Note>
  This setting is relevant only if you’re using <InlineCode>vc dev</InlineCode>{' '}
  locally to develop your project. You should be using{' '}
  <InlineCode>vc dev</InlineCode> only if you need to use a <ProductName />{' '}
...
The Development Command setting allows you to customize the behavior of <InlineCode>vc dev</InlineCode>. If <ProductName /> detects a framework, the development command will automatically be configured.

If you’d like to use a custom command for <InlineCode>vc dev</InlineCode>, you can turn on the **Override** toggle. Please note the following:
...
- If the development command is not specified, `vc dev` will fail. If you selected "Other" as the framework preset, the development command will be empty by default.
- You must create a deployment and have your local project be linked to the project on <ProductName /> (using `vc`). Otherwise, `vc dev` won’t work correctly.
...
- This setting also applies to [<ProductShortName /> CLI](/download). Instead of running `vc <directory-name>` to deploy, specify `<directory-name>` here so you can just run `vc`.
...
| --------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Production**  | When selected, the Environment Variable will be applied to your next Production Deployment. To create a Production Deployment, push a commit to the default branch or run `vc --prod`.        |
| **Preview**     | The Environment Variable is applied to your next Preview Deployment. Preview Deployments are created when you push to a branch (for example, `my-new-feature`) or run `vc`.                   |
| **Development** | The Environment Variable is for use when running your project locally, with `vc dev` or your preferred development command. To download Development Environment Variables, run `vc env pull`. |
...
Environment Variables created for the Development Environment can be downloaded into a local development setup using the `vc env pull` command provided by [<ProductShortName /> CLI](/download):

<Snippet clipboard={false} dark text={`vc env pull
Downloading Development Environment Variables for project my-lovely-project
✅ Created .env file [510ms]`} />
<Caption>Downloading Development Environment Variables with `vc env pull`.</Caption>

Running the command will create a `.env` file in your project folder. The `.env` file can then be consumed by `vc dev` or
your preferred development command.

Check out the [<ProductShortName /> CLI reference](/docs/now-cli#commands/env) to learn more about `vc env` commands.
